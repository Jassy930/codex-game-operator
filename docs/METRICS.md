...
- 远航调度闭环插画当前由静态 `index.html` 和运行时 `renderFarDispatchSceneImage()` 共同渲染为 `.far-dispatch-scene-image`，只作为本地 SVG 视觉锚点，不单独记录为真实线上指标；远航路线语义仍通过路线文本、步骤收益标签、按钮路线标记、路线反馈、反馈快照和 `directive` 事件复盘。
