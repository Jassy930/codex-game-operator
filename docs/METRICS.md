...
- 远航闭环路线微图当前由 `dispatch.loopSteps`、`loopProgress` 和 `loopTarget` 即时渲染为 `far-dispatch-loop-visual`、`--far-loop-visual-progress` 和 1/2/3 节点状态，不单独记录为真实线上指标；闭环语义仍通过远航路径文本、步骤收益标签、按钮路线标记、路线反馈、反馈快照和 `directive` 事件中的远航字段复盘。
