...
- 浏览器本地在玩家执行航线指令时记录 directive 事件，包含指令 id、本次即时收益、指令熟练收益、指令熟练倍率、当前熟练层、熟练叠层结果、预案执行奖励、航线委托奖励、远航调度奖励、远航续航奖励、远航协同奖励、远航调度闭环奖励、远航突破奖励、远航整备刷新指令、远航调度冷却倍率、远航调度连携窗口、轮换目标奖励、满层回响奖励、策略终结奖励、策略契合奖励、策略契合倍率、航线连携层数和连携倍率。
...
- 远航调度接管下一步指令推荐、目标后的远航续航推荐、协同续航指令、协同按钮“远航协同/等待协同”推荐文案、目标指令冷却缩短、调度接力窗口、远航闭环、远航突破、远航整备、星图当前航段 3 步路径轨和主操作区远航调度 3 格路径轨当前由当前航段、航线策略、冷却、连携状态与当前航段剩余进度即时推导，不单独记录为真实线上指标；执行后仍通过 `directive` 事件中的远航调度奖励、远航续航奖励、远航协同奖励、闭环奖励、远航突破奖励、整备刷新指令、冷却倍率与连携窗口字段复盘。
