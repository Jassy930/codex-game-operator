...
- 远航路线回航结果短标当前由 `branchChoices.routeReturnKind` / `routeReturnText` / `routeLoopStreakText` 即时渲染，不单独记录为真实线上指标；协同远航突破、绕行突破和 `连段 X/3` 的语义仍通过 `branchChoiceText`、卡片标题、路线下一步、收益对照、路线反馈和反馈快照复盘；连段收益与层数仍通过 `directive` 事件中的 `dispatchLoopStreakReward`、`dispatchLoopStreakRewardRate` 和 `dispatchLoopStreakStacks` 复盘。
...
- 远航路线对照条当前由 `branchChoiceSummaryText` 和 `branchChoices` 的路线徽标、当前步短标 `routePhaseText`、下一步按钮短标 `routeActionText`、收益短标 `routeRewardSummaryText`、第二步短标 `routeBranchStepText`、本步收益短标 `routePayoffSummaryText`、取向、代价、回航结果和 `routeLoopStreakText` 即时渲染，不单独记录为真实线上指标；`routeActionText` 会从 `routeCommandLabels` 带出 `下一步 1/2/3 <指令名>` 或整备指令名，但仍只是展示语义；协同/绕行路线取舍、当前阶段、下一步操作、三步回报落点、第二步按钮映射、本步即时收益差异与回航连段层数仍通过 `branchChoiceText`、卡片标题、路线收益对照、路线判断、路线指令串、路线反馈和反馈快照复盘；连段层数的事件复盘继续依赖现有 `dispatchLoopStreak*` 字段，不新增远端指标。
