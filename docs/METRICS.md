...
- 远航路线步骤按钮行动信标当前由 `dispatchRouteStepText` 派生的 `.directive-button.is-dispatch-route-step` 即时渲染为 `directiveDispatchRouteStepBeacon`，不单独记录为真实线上指标；当前步骤语义仍通过 `directive` 事件中的 `dispatchRouteStepText`、路线反馈、远航调度文本和反馈快照复盘。
