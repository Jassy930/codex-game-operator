...
- 远航路线对照条当前路线进度轨刻度层当前由 `.far-dispatch-branch-choice-summary-item.is-active-route .far-dispatch-branch-choice-summary-progress::before` 的 `repeating-linear-gradient` 背景从本地当前路线展示态即时渲染，并在降低动效偏好下关闭既有底光动画，不单独记录为真实线上指标；路线步骤、轨道推进和后续收益复盘仍依赖现有路线短标、按钮路线标记、路线反馈、反馈快照和 `directive` 事件中的远航字段。
...
- 航线委托完成节点信标当前由 `.directive-task.is-completed .directive-task-meter-node.is-complete:last-child::after` 从本地完成态即时渲染，播放 `directiveTaskCompletedNodeBeacon` 并在降低动效偏好下关闭动画；它不单独记录为真实线上指标。委托完成和收益复盘仍依赖现有 `directive` 事件中的 `taskReward`、`taskRewardRate`、执行反馈、完成收益短标和反馈快照。
...
- 航线委托下一步步号短标当前由 `getDirectiveTaskStatus().nextStepText` 从本地 `progress` / `target` 即时派生并渲染为 `.directive-task-step`，不单独记录为真实线上指标；委托推进、完成和收益复盘仍依赖现有 `directive` 事件中的 `taskReward`、`taskRewardRate`、指令轮换字段和反馈快照。
...
- 反馈状态回执视觉短标当前由本地 `.feedback-status:not(:empty)::before` CSS 展示态即时渲染，不单独记录为真实线上指标；反馈提交和校验结果仍通过现有 feedback_sent 事件、Issue 草稿和反馈快照复盘。
...
- 点火阶段光环待机信标当前由本地 `.core-button:not(.is-combo-charging):not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-stage-aura` 展示态即时渲染，不单独记录为真实线上指标；阶段进度、连击状态和过载结果仍通过 `getComboStatus`、click 事件和现有反馈快照复盘。
- 点火粒子层待机信标当前由本地 `.core-button:not(.is-combo-charging):not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-feedback-layer` 展示态即时渲染，不单独记录为真实线上指标；点火行为、命中粒子爆发、连击状态和过载结果仍通过 click 事件、`lastGain`、`lastPulse` 和现有反馈快照复盘。
...
- 点火过载状态读数待机信标当前由本地 `.core-button:not(.is-combo-charging):not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) ~ .combo-line #pulseValue` 展示态即时渲染，不单独记录为真实线上指标；过载推进意图仍通过后续 click 事件、`getComboStatus`、连击状态、过载结果和现有反馈快照复盘。
...
- 主操作行动提示和离线回收提示视觉短标当前由本地 `.action-notice::before` 与 `.offline-notice::before` CSS 展示态即时渲染，不单独记录为真实线上指标；行动结果仍通过 click、upgrade_purchase、directive、goal_complete、offline_gain 和现有反馈快照复盘。
...
- 远航主调度条满段回响预告当前由 `getFarRouteDispatch()` 派生 `loopCapstoneText`，并由 `.far-dispatch-loop-capstone` / `.far-dispatch.has-loop-capstone` 即时渲染，不单独记录为真实线上指标；满段回响收益复盘仍通过 `directive` 事件中的 `dispatchLoopStreakCapstoneReward` 和 `dispatchLoopStreakCapstoneRewardRate` 字段。
...
- 远航路线回航结果短标当前由 `branchChoices.routeReturnKind` / `routeReturnText` / `routeLoopStreakText` / `routeLoopCapstoneText` 即时渲染，不单独记录为真实线上指标；协同远航突破、绕行突破、`连段 X/3` 和 `满段回响 +10%` 的语义仍通过 `branchChoiceText`、卡片标题、路线下一步、收益对照、路线反馈和反馈快照复盘；卡片标题和 `branchChoiceText` 必须复用 `routeReturnText` 作为满段回响文案唯一来源，避免二次拼接；满段回响预告只改变本地展示，连段与满段收益复盘仍通过 `directive` 事件中的 `dispatchLoopStreakReward`、`dispatchLoopStreakRewardRate`、`dispatchLoopStreakStacks`、`dispatchLoopStreakCapstoneReward` 和 `dispatchLoopStreakCapstoneRewardRate` 字段复盘。
...
- 远航路线对照条当前由 `branchChoiceSummaryText` 和 `branchChoices` 的路线徽标、当前步短标 `routePhaseText`、下一步按钮短标 `routeActionText`、收益短标 `routeRewardSummaryText`、第二步短标 `routeBranchStepText`、本步收益短标 `routePayoffSummaryText`、取向、代价、回航结果、`routeLoopStreakText` 和 `routeLoopCapstoneText` 即时渲染，不单独记录为真实线上指标；`routeActionText` 会从 `routeCommandLabels` 带出 `下一步 1/2/3 <指令名>` 或整备指令名，但仍只是展示语义；协同/绕行路线取舍、当前阶段、下一步操作、三步回报落点、第二步按钮映射、本步即时收益差异、回航连段层数与满段回响预告仍通过 `branchChoiceText`、卡片标题、路线收益对照、路线判断、路线指令串、路线反馈和反馈快照复盘；连段层数和满段回响收益的事件复盘继续依赖现有 `dispatchLoopStreak*` 字段，不新增远端指标。
- 远航路线对照条决策信标状态点当前由 `.far-dispatch-branch-choice-summary-signal::before` 和 `routeDecisionSignalKind` 对应样式即时渲染，当前路线信标的 `farDispatchSummarySignalBeacon` 只作为本地展示动效，不单独记录为真实线上指标；路线定位语义仍由 `routeDecisionSignalText`、路线判断、路线对照摘要、按钮路线标记、路线反馈和反馈快照复盘。
...
- 浏览器本地通过 localStorage 保存最近 50 条 feedback 草稿，提交反馈时记录 feedback_sent 事件；反馈快照包含当前过载奖励、航线策略、指令熟练层和远航调度状态；当 `getFarRouteDispatch()` 返回有效 `loopStreakText` 时，Issue 草稿会额外输出 `远航连段：连段 X/3`，该短标仍是本地快照内容，不代表远端指标上报。
