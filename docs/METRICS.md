...
- 浏览器本地在玩家执行航线指令时记录 directive 事件，包含指令 id、本次即时收益、指令熟练收益、指令熟练倍率、当前熟练层、熟练叠层结果、预案执行奖励、航线委托奖励、远航调度奖励、远航调度冷却倍率、轮换目标奖励、满层回响奖励、策略终结奖励、策略契合奖励、策略契合倍率、航线连携层数和连携倍率。
- 指令轮换目标当前由本地冷却与连携字段即时推导，不单独记录为真实线上指标。
- 远航调度接管下一步指令推荐和目标指令冷却缩短当前由当前航段、航线策略和冷却状态即时推导，不单独记录为真实线上指标；执行后仍通过 `directive` 事件中的远航调度奖励与冷却倍率字段复盘。
