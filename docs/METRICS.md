...
- 远航闭环路线微图当前由 `dispatch.loopSteps`、`loopProgress` 和 `loopTarget` 即时渲染为 `far-dispatch-loop-visual`、`--far-loop-visual-progress`、1/2/3 节点状态、当前节点脉冲光标、1 -> 2 / 2 -> 3 方向箭头、当前方向信标和 `far-dispatch-loop-visual-reward` 收益短标；箭头状态由 `getFarDispatchLoopVisualArrowState` 从闭环进度派生，收益短标由 `getFarDispatchLoopVisualRewardKind` / `getFarDispatchLoopVisualRewardLabel` 从步骤位置与标签派生，当前方向信标由 `.far-dispatch-loop-visual-arrow.is-current` 即时渲染，当前节点脉冲由 `.far-dispatch-loop-visual-node.is-current::after` 即时渲染；下方远航闭环步骤卡步号锚点和收益色带复用同一组 `loopSteps`、`getFarDispatchLoopVisualRewardKind` 和 `getFarDispatchLoopVisualRewardLabel`，由 `data-step-label`、`far-dispatch-loop-step is-*`、`far-dispatch-step-reward is-*`、`data-reward-label` 与 CSS 伪元素/样式即时渲染，不单独记录为真实线上指标；闭环语义仍通过远航路径文本、步骤收益标签、按钮路线标记、路线反馈、反馈快照和 `directive` 事件中的远航字段复盘。
