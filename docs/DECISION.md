...
当前最大问题：远航路线对照条已经把当前路线置顶、加宽、整卡补光，并在 1/2/3 迷你进度轨里标出底光、已推进端点、已完成节点回响和下一步节点外环；但待推进节点仍保持普通暗点。玩家能看到“已走过”和“下一步”，但后续待推进节点与整条路线链路之间还缺少一层不增加文字的预告。

本轮决策：

- 新增“远航对照条当前路线待推进节点预告环”。
- `src/styles.css` 仅对 `.far-dispatch-branch-choice-summary-item.is-active-route .far-dispatch-branch-choice-summary-progress-node.is-waiting::after` 增加轻量虚线预告环和 `farDispatchSummaryWaitingNodeBeacon`。
- 绕行路线复用 `.far-dispatch-branch-choice-summary-progress.is-detour .far-dispatch-branch-choice-summary-progress-node.is-waiting::after` 切换粉色预告环，继续保持协同 / 绕行图形差异。
- `prefers-reduced-motion: reduce` 必须关闭待推进节点预告环动画，保留静态待推进节点。
- `tests/game.test.js` 增加静态断言，覆盖当前路线待推进节点预告环、绕行配色、关键帧和降低动效兜底。
- 该改动只增强当前路线 1/2/3 进度轨待推进节点扫视，不新增 DOM、不新增界面可见文字、不新增收益、不新增存档字段，不改变远航收益、路线选择、指令冷却、星图航段、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-06 09:34 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#6 作为本轮主关联反馈，#4 作为图形化和文字密度约束。
- 当前路线对照槽内的 `.far-dispatch-branch-choice-summary-progress-node.is-waiting::after` 获得轻量虚线预告环；绕行路线使用单独粉色预告环；降低动效偏好下关闭动画。
...
- `9820b0a feat: highlight pending far route nodes` 已推送；GitHub Pages workflow `25411924715` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含待推进节点预告环样式。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 #6 和 #4，issue 均保持 open 等复测；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4384480732`、`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4384481677`。回复后于 2026-05-06 09:42 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#6 更新时间为 `2026-05-06T01:41:58Z`，#4 更新时间为 `2026-05-06T01:42:15Z`。
- 钉钉通知未发送：2026-05-06 09:43 CST 运行环境未提供 `DING` / `DINGTALK` / `WEBHOOK` / `ROBOT` 相关有效 webhook 变量，当前目录和 `/home/jassy/glm` 两层内未发现 `.env*` 文件；未将 webhook 写入仓库。
- 本轮不新增外部网页调研；依据来自真实 GitHub 反馈 #6/#4，以及当前路线已完成节点、进度端点和下一步节点已增强但待推进节点仍偏普通暗点的扫视链路复盘。

## 2026-05-06 Product decision：远航对照条当前路线已完成节点回响

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-06 09:19 CST 已同步到 5 个 open issue、5 个 open feedback issue、0 个 open bug issue。没有 open bug；#6“后半段玩法没有真正变化”仍是主反馈，#4 的文字密度和图片化反馈继续约束本轮不能新增说明文字。

当前最大问题：远航路线对照条已经把当前路线置顶、加宽、整卡补光，并在 1/2/3 迷你进度轨里标出底光、已推进端点和下一步节点外环；但已完成节点本身仍只是静态实心点。玩家能看到当前端点和下一步，但已走过的节点与当前路线进展之间还缺少一层不增加文字的回响。

本轮决策：

- 新增“远航对照条当前路线已完成节点回响”。
- `src/styles.css` 仅对 `.far-dispatch-branch-choice-summary-item.is-active-route .far-dispatch-branch-choice-summary-progress-node.is-done::after` 增加轻量完成外环和 `farDispatchSummaryDoneNodeEcho`。
- 绕行路线复用 `.far-dispatch-branch-choice-summary-progress.is-detour .far-dispatch-branch-choice-summary-progress-node.is-done::after` 切换粉色回响，继续保持协同 / 绕行图形差异。
- `prefers-reduced-motion: reduce` 必须关闭完成节点回响动画，保留静态完成节点。
- `tests/game.test.js` 增加静态断言，覆盖当前路线已完成节点回响、绕行配色、关键帧和降低动效兜底。
- 该改动只增强当前路线 1/2/3 进度轨已完成节点扫视，不新增 DOM、不新增界面可见文字、不新增收益、不新增存档字段，不改变远航收益、路线选择、指令冷却、星图航段、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-06 09:19 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#6 作为本轮主关联反馈，#4 作为图形化和文字密度约束。
- 当前路线对照槽内的 `.far-dispatch-branch-choice-summary-progress-node.is-done::after` 获得轻量外环回响；绕行路线使用单独粉色回响；降低动效偏好下关闭动画。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 135 项。
...
- `430c7ed feat: highlight completed far route nodes` 已推送；GitHub Pages workflow `25411556168` 成功，线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含完成节点回响样式。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 #6 和 #4，issue 均保持 open 等复测；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4384438802`、`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4384439569`。回复后于 2026-05-06 09:29 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#6 更新时间为 `2026-05-06T01:28:47Z`，#4 更新时间为 `2026-05-06T01:29:04Z`。
- 钉钉通知未发送：2026-05-06 09:29 CST 运行环境未提供有效 webhook 变量，当前目录和 `/home/jassy/glm` 两层内未发现 `.env*` 文件；未将 webhook 写入仓库。
- 本轮不新增外部网页调研；依据来自真实 GitHub 反馈 #6/#4，以及当前路线端点光已增强但已完成节点仍偏静态的扫视链路复盘。

## 2026-05-06 Product decision：远航对照条当前路线进度端点光

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-06 09:05 CST 已同步到 5 个 open issue、5 个 open feedback issue、0 个 open bug issue。没有 open bug；#6“后半段玩法没有真正变化”仍是主反馈，#4 的文字密度和图片化反馈继续约束本轮不能新增说明文字。

当前最大问题：远航路线对照条已经把当前路线置顶、加宽、整卡补光，并在 1/2/3 迷你进度轨里标出底光、已推进填充和下一步节点外环；但已推进填充段的末端仍较平直，玩家扫当前路线进度时可能看见轨道，却不容易一眼定位“已推进到这里”的端点。

本轮决策：

- 新增“远航对照条当前路线进度端点光”。
- `src/styles.css` 仅对 `.far-dispatch-branch-choice-summary-item.is-active-route .far-dispatch-branch-choice-summary-progress::after` 增加已推进填充段末端亮边和端点光。
- 绕行路线复用 `.far-dispatch-branch-choice-summary-progress.is-detour::after` 切换粉色端点光，继续保持协同 / 绕行图形差异。
- `tests/game.test.js` 增加静态断言，覆盖当前路线填充端点亮边、协同端点光和绕行端点光。
- 该改动只增强当前路线 1/2/3 进度轨的已推进端点扫视，不新增 DOM、不新增界面可见文字、不新增收益、不新增存档字段，不改变远航收益、路线选择、指令冷却、星图航段、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-06 09:05 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#6 作为本轮主关联反馈，#4 作为图形化和文字密度约束。
...
- 发布和反馈处理已完成：提交 `1495f2e feat: highlight active far route rail` 已推送；GitHub Pages workflow `25410688244` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.far-dispatch-branch-choice-summary-progress::before` 和 `farDispatchSummaryActiveRouteRailBeacon`。
- 已回复 GitHub Issue #6 和 #4，说明当前路线进度轨底光、验证结果、Pages 部署和复测问题；issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4384327836`、`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4384329016`。回复后于 2026-05-06 08:58 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#6 更新时间为 `2026-05-06T00:57:54Z`，#4 更新时间为 `2026-05-06T00:58:09Z`。
...
当前最大问题：远航路线对照条已经把当前路线置顶、加宽、整卡补光，并在 1/2/3 迷你进度轨里标出下一步节点；但下一步节点主要依赖节点本体放大，玩家在当前路线宽栏内仍可能先扫到动作、收益或回航结果，最后才定位到进度轨上的当前执行点。

本轮决策：

- 新增“远航对照条下一步节点外环信标”。
- `src/styles.css` 仅对 `.far-dispatch-branch-choice-summary-item.is-active-route .far-dispatch-branch-choice-summary-progress-node.is-next::after` 增加外环信标和 `farDispatchSummaryNextNodeHalo` 动画。
- 绕行路线复用 `.far-dispatch-branch-choice-summary-progress.is-detour` 切换粉色外环，保持协同 / 绕行图形差异。
- `prefers-reduced-motion: reduce` 必须关闭下一步外环动画，保留静态下一步节点。
- `tests/game.test.js` 增加静态断言，覆盖下一步外环、绕行配色、关键帧和降低动效兜底。
- 该改动只增强当前路线 1/2/3 进度轨的下一步扫视，不新增 DOM、不新增界面可见文字、不新增收益、不新增存档字段，不改变远航收益、路线选择、指令冷却、星图航段、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-06 08:34 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#6 作为本轮主关联反馈，#4 作为文字密度约束。
- 当前路线对照槽内的 `.far-dispatch-branch-choice-summary-progress-node.is-next` 获得外环信标；绕行路线使用单独粉色外环；降低动效偏好下关闭动画。
- 本地验证已通过：`npm test`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 135 项。
...
- 发布和反馈处理已完成：提交 `6d76f92 feat: highlight far route next node` 已推送；GitHub Pages workflow `25410250781` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.far-dispatch-branch-choice-summary-progress-node.is-next::after` 和 `farDispatchSummaryNextNodeHalo`。
- 已回复 GitHub Issue #6 和 #4，说明下一步节点外环信标、验证结果、Pages 部署和复测问题；issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4384273271`、`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4384273279`。回复后于 2026-05-06 08:42 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#6/#4 更新时间为 `2026-05-06T00:42:20Z`。
...
- 本轮不新增外部网页调研；依据来自真实 GitHub 反馈 #6/#4，以及当前路线下一步节点已有放大但缺少外环锚点的扫视链路复盘。
...
- 发布验证已通过：提交 `c8ff7be` 已推送，GitHub Pages workflow `25405623395` 成功，线上首页、`src/app.js` 和 `src/styles.css` 均返回 HTTP 200，线上文件已确认包含本轮标记。
- 已回复 GitHub Issue #4，说明升级插画购买态投光、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4383614475`。回复后于 2026-05-06 06:25 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#4 更新时间为 `2026-05-05T22:25:19Z`。
...
- 发布验证已通过：GitHub Pages workflow `25401500495` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 已确认包含 `directivePlanTrack.classList.toggle("is-next-ready"`、`is-next-waiting`，线上 `src/styles.css` 已确认包含 `.directive-plan-step.is-next-ready`、`.directive-plan-step.is-next-waiting` 与 `directivePlanNextReadyBeacon`。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3、#4 和 #6，说明指令轮换下一步格可执行信标、验证结果、Pages 部署和复测问题；三个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4382951610`、`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4382951548`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4382951552`。
- 回复后于 2026-05-06 04:51 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 更新时间为 `2026-05-05T20:51:18Z`，#4/#6 更新时间为 `2026-05-05T20:51:17Z`。
...
- 代码和同步文档提交已创建并推送：`dbaaf62 feat: show directive task completion followup`。
- 发布验证已通过：GitHub Pages workflow `25399988590` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js`、`src/game.js` 和 `src/styles.css` 已确认包含 `completedFollowupText`、`航线委托完成续航动作` 与 `.directive-task-action.is-completed`。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明航线委托完成续航短标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4382703173`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4382703324`。
- 回复后于 2026-05-06 04:18 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 更新时间为 `2026-05-05T20:18:32Z`，#6 更新时间为 `2026-05-05T20:18:34Z`。
- 钉钉通知未发送：2026-05-06 04:18 CST 运行环境未提供 `DING` / `DINGTALK` / `WEBHOOK` / `ROBOT` 相关有效 webhook URL，当前目录和 `/home/jassy/glm` 两层内未发现 `.env*` 文件；未将 webhook 写入仓库。
...
当前最大问题：航线委托 3/3 完成态已经显示完成收益短标、最终节点信标、整卡完成面板信标和进度扫光，但完成后步号短标隐藏。玩家能看到满格轨道，却不能在同一短标层直接扫到“这轮已经到 3/3 终点”。

本轮决策：

- 新增“航线委托完成步号短标”。
- `src/game.js` 在 completed 任务状态中返回 `completedStepText: "3/3 完成"`。
- `src/app.js` 在 `renderDirectiveTask()` 中让完成态复用步号胶囊显示 `3/3 完成`，并写入“航线委托完成步号”标题和可访问标签。
- `src/styles.css` 增加 `.directive-task-step.is-completed` 完成态样式。
- `tests/game.test.js` 覆盖状态字段、运行时绑定、完成步号标题和样式。
- 该改动只增强 100K 后 3/3 委托完成态扫视，不新增收益、不新增存档字段，不改变指令冷却、连携窗口、策略契合、航线委托奖励数值、星图航段、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-06 03:52 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#3/#6 作为本轮关联反馈。
- 航线委托完成态会在完成收益短标旁显示 `3/3 完成`，未完成态继续显示下一步步号，锁定态隐藏。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 129 项。
...
- 发布验证已通过：GitHub Pages workflow `25399198245` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js`、`src/game.js` 和 `src/styles.css` 已确认包含 `completedStepText`、`航线委托完成步号` 与 `.directive-task-step.is-completed`。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明航线委托完成步号短标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4382589100`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4382589127`。
- 回复后于 2026-05-06 04:02 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 更新时间为 `2026-05-05T20:02:33Z`，#6 更新时间为 `2026-05-05T20:02:34Z`。
...
- 本轮未新增外部网页调研；依据来自真实 GitHub 反馈 #3/#6，以及航线委托完成态已有图形反馈但缺少完成步号短标的链路复盘。
...
- 代码和同步文档提交已创建并推送：`d7313d3 feat: add directive task completion meter sweep`。
- 发布验证已通过：GitHub Pages workflow `25398312221` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.directive-task.is-completed .directive-task-meter-fill::after` 与 `directiveTaskCompletedMeterSweep`。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明航线委托完成进度扫光、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4382444062`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4382445476`。
- 回复后于 2026-05-06 03:44 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 更新时间为 `2026-05-05T19:43:54Z`，#6 更新时间为 `2026-05-05T19:44:09Z`。
- 钉钉通知未发送：2026-05-06 03:44 CST 运行环境未提供 `DING` / `DINGTALK` / `WEBHOOK` / `ROBOT` 相关有效 webhook URL，当前目录和 `/home/jassy/glm` 两层内未发现 `.env*` 文件；未将 webhook 写入仓库。
...
当前最大问题：航线委托 3/3 完成态已经保留 `委托完成 +8%` 完成收益短标，但 1/2/3 三个完成节点都只是同一种静态完成色。玩家扫委托卡时能看到完成收益胶囊，却还缺少一层不增加文字密度的最终节点信号，把“这是三步终点”与“本轮完成收益”连起来。

本轮决策：

- 新增“航线委托完成节点信标”。
- `src/styles.css` 为 `.directive-task.is-completed .directive-task-meter-node.is-complete:last-child::after` 增加青绿色完成外环、轻量补光和 `directiveTaskCompletedNodeBeacon` 动画。
- `prefers-reduced-motion: reduce` 下关闭完成节点动画，保留静态完成节点展示。
- `tests/game.test.js` 覆盖完成节点选择器、动画 keyframes 和降低动效兜底。
- 该改动只增强 100K 后 3/3 委托完成回报扫视，不新增可见文字、不新增收益、不新增存档字段，不改变指令冷却、连携窗口、策略契合、航线委托奖励数值、星图航段、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-06 02:57 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#3/#6 作为本轮关联反馈。
- 航线委托完成态最后一个 3/3 节点显示完成外环和轻量脉冲；降低动效偏好下关闭脉冲动画。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 129 项。
- 构建产物已确认 `dist/src/styles.css` 包含 `.directive-task.is-completed .directive-task-meter-node.is-complete:last-child::after` 与 `directiveTaskCompletedNodeBeacon`。
- 代码和同步文档提交已创建并推送：`1fc0d5f feat: highlight directive task completion node`。
- 发布验证已通过：GitHub Pages workflow `25396571465` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.directive-task.is-completed .directive-task-meter-node.is-complete:last-child::after` 与 `directiveTaskCompletedNodeBeacon`。
- 已回复 GitHub Issue #3 和 #6，说明航线委托完成节点信标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4382203971`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4382205930`。
- 回复后于 2026-05-06 03:10 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 更新时间为 `2026-05-05T19:07:28Z`，#6 更新时间为 `2026-05-05T19:07:45Z`。
- 钉钉通知未发送：2026-05-06 03:10 CST 运行环境未提供 `DING` / `DINGTALK` / `WEBHOOK` / `ROBOT` 相关有效 webhook URL，当前目录和 `/home/jassy/glm` 两层内未发现 `.env*` 文件；未将 webhook 写入仓库。
- 本轮未新增外部网页调研；依据来自真实 GitHub 反馈 #3/#6，以及航线委托完成态最终节点缺少完成信标的扫视链路复盘。

## 2026-05-06 Product decision：航线委托完成收益短标

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-06 02:39 CST 已同步到 5 个 open issue、5 个 open feedback issue、0 个 open bug issue。没有 open bug；#3“玩法太简单”和 #6“后半段玩法没有真正变化”仍是最近需要持续复测的主动玩法反馈。

当前最大问题：航线委托条已经显示下一步步号、意图、动作、状态、收益、可执行面板信标和 1/2/3 节点信标。但 3/3 委托完成后，默认可见层只剩“已完成”和满格进度条，`委托完成 +8%` 主要留在执行反馈或完整悬停说明里。玩家扫委托卡时不容易把刚完成的三步短循环和本轮完成奖励对应起来。

本轮决策：

- 新增“航线委托完成收益短标”。
- `index.html` 为静态首页委托区预留隐藏的 `directive-task-reward` 节点。
- `src/app.js` 在 `renderDirectiveTask()` 中用 `task.completed ? task.rewardText : task.nextRewardText` 决定收益短标；完成态追加 `is-completed` 样式，并把标题与可访问标签写为“航线委托完成收益”。
- `src/styles.css` 为 `.directive-task-reward.is-completed` 增加青绿色完成态边框、底色和轻量补光。
- `tests/game.test.js` 覆盖 completed 状态的 `rewardText`、静态首页隐藏节点、运行时绑定和 CSS。
- 该改动只增强 100K 后 3/3 委托完成回报扫视，不新增收益、不新增存档字段，不改变指令冷却、连携窗口、策略契合、航线委托奖励数值、星图航段、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-06 02:39 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#3/#6 作为本轮关联反馈。
...
- 发布验证已通过：GitHub Pages workflow `25394859070` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页、`src/app.js` 和 `src/styles.css` 均返回 HTTP 200，线上 JS/CSS 已确认包含 `is-next-ready`、`is-next-waiting` 与 `directiveTaskReadyPanelBeacon`。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明航线委托可执行面板信标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4381968699`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4381971371`。
- 回复后于 2026-05-06 02:33 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 更新时间为 `2026-05-05T18:33:18Z`，#6 更新时间为 `2026-05-05T18:33:40Z`。
...
- 代码提交已创建并推送：`87a5daf feat: add upgrade panel visual asset`。
- 发布验证已通过：GitHub Pages workflow `25393172473` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页、`src/styles.css` 和 `src/assets/upgrade-visual.svg` 均返回 HTTP 200，并已确认包含 `upgrade-scene-image`、`upgrade-visual.svg`、`升级工坊视觉插画` 与 `upgradeRail`。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明升级面板本地插画、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4381740966`。
...
- 代码提交已创建并推送：`04d02b7 feat: show directive task next intent`。
- 发布验证已通过：GitHub Pages workflow `25391777653` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页、`src/app.js`、`src/game.js` 和 `src/styles.css` 均返回 HTTP 200，并已确认包含 `directive-task-intent`、`nextIntentText`、`buildDirectiveTaskNextIntentText` 与“航线委托下一步意图”。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明航线委托下一步意图短标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4381549382`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4381548542`。
...
- 代码提交已创建并推送：`d1c2291 feat: show directive task next step`。
- 发布验证已通过：GitHub Pages workflow `25390784667` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页、`src/app.js`、`src/game.js` 和 `src/styles.css` 均返回 HTTP 200，并已确认包含 `directive-task-step`、`nextStepText` 与“航线委托下一步步号”。
- 已回复 GitHub Issue #3 和 #6，说明航线委托下一步步号短标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4381416477`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4381416702`。
- 回复后于 2026-05-06 01:09 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 更新时间为 `2026-05-05T17:09:22Z`，#6 更新时间为 `2026-05-05T17:09:23Z`。
- 钉钉通知未发送：2026-05-06 01:10 CST 运行环境未提供 `DING` / `DINGTALK` / `WEBHOOK` / `ROBOT` 相关有效 webhook URL，当前目录和 `/home/jassy/glm` 两层内未发现 `.env*` 文件；未将 webhook 写入仓库。
...
当前最大问题：航线委托条已经同层显示下一步动作、下一步收益和 1/2/3 节点，但玩家看到“下一步 点火齐射或巡航回收”后，仍要回到按钮区确认这些指令是否可立即执行，或是否还在冷却。

本轮决策：

- 新增“航线委托下一步状态短标”。
- `src/game.js` 让 `getDirectiveTaskStatus()` 输出 `nextStatusText` / `nextStatusKind`，从现有下一步推荐指令冷却状态派生 `可执行` 或 `等待 X 秒`。
- `src/app.js` 在 `renderDirectiveTask()` 中渲染 `directive-task-status`，并写入“航线委托下一步状态”的标题和可访问标签。
- `index.html` 为静态首页委托区预留锁定态隐藏 status 节点。
- `src/styles.css` 为可执行和等待冷却提供不同状态色。
- `tests/game.test.js` 覆盖锁定态、可执行态、等待冷却态、完成态、静态首页、运行时绑定、CSS 和构建标记。
- 该改动只增强航线委托下一步可执行性可见性，不新增收益、不新增存档字段，不改变指令冷却、连携窗口、策略契合、航线委托奖励、远航调度、星图航段、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-06 00:34 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#3/#6 作为本轮关联反馈。#3 更新时间为 `2026-05-05T16:26:13Z`，#6 更新时间为 `2026-05-05T16:26:15Z`。
- 航线委托 0/3 下一步可执行时显示 `可执行`；推荐指令全部冷却时显示最近可执行等待时间，例如 `等待 34 秒`；完成态和锁定态不显示状态短标。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 128 项。
...
- 代码提交已创建并推送：`817ea59 feat: show directive task next status`。
- 发布验证已通过：GitHub Pages workflow `25389878179` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页、`src/app.js`、`src/game.js` 和 `src/styles.css` 均返回 HTTP 200，并已确认包含 `directive-task-status`、`nextStatusText`、`nextStatusKind` 与“航线委托下一步状态”。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明航线委托下一步状态短标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4381285514`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4381285582`。
- 回复后于 2026-05-06 00:50 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 和 #6 更新时间均为 `2026-05-05T16:50:52Z`。
- 钉钉通知未发送：2026-05-06 00:51 CST 运行环境未提供 `DING` / `DINGTALK` / `WEBHOOK` / `ROBOT` 相关有效 webhook URL；未将 webhook 写入仓库。
- 本轮未新增外部网页调研；依据来自真实 GitHub 反馈 #3/#6，以及航线委托条已有动作/收益/节点但缺少下一步可执行状态的链路复盘。

## 2026-05-06 Product decision：航线委托下一步动作短标

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-06 00:14 CST 已同步到 5 个 open issue、5 个 open feedback issue、0 个 open bug issue。没有 open bug；#3“玩法太简单”和 #6“后半段玩法没有真正变化”仍是本轮关联反馈。

当前最大问题：航线委托已经有下一步收益短标、三节点进度条和 1/2/3 节点步号，但委托条默认可见层没有单独展示“下一步按哪个指令”。完整动作只藏在 `task.text` 的 title / aria 中，玩家仍要从上方指令轮换轨或按钮推荐回看下一步动作。

本轮决策：

- 新增“航线委托下一步动作短标”。
- `src/game.js` 让 `getDirectiveTaskStatus()` 输出 `nextActionText`，复用既有下一步推荐指令文本。
- `src/app.js` 在 `renderDirectiveTask()` 中渲染 `directive-task-action`，并把标题和可访问标签写为“航线委托下一步动作：...”。
- `index.html` 为静态首页委托区预留锁定态隐藏 action 节点。
- `src/styles.css` 让动作短标使用独立紫色胶囊，并和收益短标共用单行截断布局。
- `tests/game.test.js` 覆盖锁定态、起手态、2/3 收束、完成态、静态首页、运行时绑定和 CSS。
- 该改动只增强航线委托下一步动作可见性，不新增收益、不新增存档字段，不改变指令冷却、连携窗口、策略契合、航线委托奖励、远航调度、星图航段、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-06 00:14 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#3/#6 作为本轮关联反馈。#3 更新时间为 `2026-05-05T16:07:35Z`，#6 更新时间为 `2026-05-05T16:05:36Z`。
- 航线委托 0/3 会在委托条内显示 `下一步 点火齐射或巡航回收`，2/3 会显示 `下一步 谐振脉冲`，完成态不显示动作短标。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 128 项。
...
- 发布验证已通过：GitHub Pages workflow `25388637741` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页、`src/app.js`、`src/game.js` 和 `src/styles.css` 均返回 HTTP 200，并已确认包含 `directive-task-action`、`nextActionText` 与“航线委托下一步动作”。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明航线委托下一步动作短标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4381121405`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4381121558`。
...
- 代码提交已创建并推送：`dfb9c8d feat: show directive task progress nodes`。
- 发布验证已通过：GitHub Pages workflow `25386816696` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上首页、`src/app.js` 和 `src/styles.css` 已确认包含 `directive-task-meter-fill` 与 `directive-task-meter-node`。workflow 给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明航线委托三节点进度条、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4380866523`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4380868964`。
- 回复后于 2026-05-05 23:50 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 更新时间为 `2026-05-05T15:49:52Z`，#6 更新时间为 `2026-05-05T15:50:13Z`。
...
当前最大问题：上一轮已经让指令轮换三格轨显示下一步收益，但航线委托条仍主要显示 `航线委托 X/3` 和完成奖励。玩家如果从委托条理解三步任务，只能看到进度和最终奖励，不能在同一处看到下一步会触发的预案执行、连携、轮换目标、策略终结或满层回响；2/3 收束时也需要从按钮预览或执行反馈里拼出 `委托完成 +8%`。

本轮决策：

- 新增“航线委托下一步收益短标”。
- `src/game.js` 让 `getDirectiveTaskStatus()` 输出 `nextRewardText`，复用 `getDirectivePlan()` 的下一步收益；当下一步会完成 3/3 委托时追加既有 `委托完成 +8%`。
- `src/app.js` 在 `renderDirectiveTask()` 中渲染 `directive-task-reward`，并让紧凑文本优先显示 `task.nextRewardText`。
- `src/styles.css` 新增 `.directive-task-reward`，用单行截断避免长收益串撑开委托条。
- `tests/game.test.js` 覆盖锁定态、起手态、2/3 收束、完成态、运行时绑定和 CSS。
- 该改动只增强航线委托收益可见性，不新增收益、不新增存档字段，不改变指令冷却、连携窗口、策略契合、远航调度、星图航段、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-05 23:17 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#3/#6 作为本轮关联反馈。回复后于 2026-05-05 23:28 CST 再次同步，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3/#6 更新时间均为 `2026-05-05T15:28:17Z`。
- 航线委托 0/3 会显示 `预案执行 +6%`，2/3 会显示 `连携 +24% · 轮换目标 +18% · 策略终结 +12% · 委托完成 +8%`。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 128 项。
...
- 代码提交已创建并推送：`a136a84 feat: show directive task reward hints`。
- 发布验证已通过：GitHub Pages workflow `25385640141` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页、`src/app.js`、`src/game.js` 和 `src/styles.css` 均返回 HTTP 200，线上产物已确认包含 `directive-task-reward`、`buildDirectiveTaskNextRewardText` 与 `下一步收益`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明航线委托下一步收益短标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4380712436`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4380712460`。
...
- 代码提交已创建并推送：`20300f1 feat: show directive plan reward hints`。
- 发布验证已通过：GitHub Pages workflow `25384570027` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页、`src/app.js`、`src/game.js` 和 `src/styles.css` 均返回 HTTP 200，线上产物已确认包含 `nextRewardText` 与 `directive-plan-step-reward`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #3 和 #6，说明指令轮换下一步收益短标、验证结果、Pages 部署和复测问题；两个 issue 均保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/3#issuecomment-4380554412`、`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4380554495`。
- 回复后于 2026-05-05 23:08 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#3 更新时间为 `2026-05-05T15:08:09Z`，#6 更新时间为 `2026-05-05T15:08:10Z`。
...
- 代码和同步文档提交已创建并推送：`5b4cd54 feat: show far capstone in star overview`。
- 发布验证已通过：GitHub Pages workflow `25382724252` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/game.js` 已确认包含 `dispatch.loopCapstoneText` 和 `loopCapstoneText +` 的星图总览摘要追加逻辑。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航满段回响星图总览短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4380284852`。
- 回复后于 2026-05-05 22:34 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#6 更新时间为 `2026-05-05T14:34:00Z`。
...
当前最大问题：远航满段回响已经进入按钮预览、执行反馈、路线对照、卡片提示和主调度条，但游戏内反馈 Issue 草稿只独立记录 `远航连段：连段 X/3`。如果玩家在即将触发满段回响时提交复测反馈，维护者仍需要从远航调度长句里反推是否处于满段预告状态。

本轮决策：

- 新增“远航满段回响反馈快照短标”。
- `src/feedback.js` 从 `getFarRouteDispatch()` 读取 `loopCapstoneText`，写入 `snapshot.farRouteLoopCapstone`。
- `createFeedbackIssueBody()` 在该值存在时追加 `- 远航满段回响：满段回响 +10%`。
- `tests/game.test.js` 覆盖当前远航闭环下一步回目标会触发满段回响时，反馈快照同时记录 `远航连段：连段 2/3` 和独立满段回响预告行。
- 该改动只增强反馈快照可诊断性，不新增收益、不新增存档字段，不改变远航连段结算、满段回响奖励、目标指令、协同/绕行路线、冷却、连携窗口、星图航段、反馈入口交互或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-05 22:10 CST 当前 5 个 open issue、5 个 open feedback issue、0 个 open bug issue；#6 作为本轮主处理对象。
- 20M 后当前远航闭环下一步回目标会触发满段回响时，反馈 Issue 草稿包含 `- 远航满段回响：满段回响 +10%`。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 128 项。
- 构建产物已确认 `dist/src/feedback.js` 包含 `farRouteLoopCapstone` 与 `- 远航满段回响：`。
- 代码和同步文档提交已创建并推送：`e843e33 feat: record far capstone in feedback snapshot`。
- 发布验证已通过：GitHub Pages workflow `25381894544` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/feedback.js` 已确认包含 `farRouteLoopCapstone` 和 `远航满段回响` 快照行。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航满段回响反馈快照短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4380167324`。
- 回复后于 2026-05-05 22:18 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#6 更新时间为 `2026-05-05T14:18:30Z`。
...
阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-05 21:48 CST 已同步到 5 个 open feedback issue、0 个 open bug issue，回复后于 2026-05-05 22:04 CST 再次同步。没有 open bug；本轮继续处理真实反馈 #6“到后半段，游戏玩法已经开始无聊了，只有不停的目标，但是游戏玩法没有真正的变化”。
...
- 发布验证已通过：GitHub Pages workflow `25381125865` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/game.js` 已确认包含 `loopCapstoneText`，`src/app.js` 已确认包含 `far-dispatch-loop-capstone` / `has-loop-capstone`，`src/styles.css` 已确认包含 `.far-dispatch.has-loop-capstone`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航满段回响主调度条预告、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4380017388`。
- 回复后于 2026-05-05 22:04 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#6 更新时间为 `2026-05-05T14:04:32Z`。
...
- 发布验证已通过：GitHub Pages workflow `25379973414` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/game.js` 已确认包含满段回响逻辑，且不再包含 `routeLoopCapstoneText ? " · "` 二次拼接或 `满段回响 +10% · 满段回响 +10%` 重复短语。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航满段回响卡片提示去重、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4379830054`。
- 回复后于 2026-05-05 21:43 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#6 更新时间为 `2026-05-05T13:42:51Z`。
...
- 发布验证已通过：GitHub Pages workflow `25378979386` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/game.js` 已确认包含 `routeLoopCapstoneText` 与 `满段回响`，`src/app.js` 已确认包含 `has-loop-capstone`，`src/styles.css` 已确认包含 `farDispatchSummaryLoopCapstoneResultGlow`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航满段回响路线对照预告、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4379660831`。
- 回复后于 2026-05-05 21:24 CST 再次同步 GitHub Issues，当前仍为 5 个 open feedback issue、0 个 open bug issue；#6 更新时间为 `2026-05-05T13:23:54Z`。
...
当前最大问题：远航路线对照条已经能直接显示航段契合、新路线、稳航、改道、当前路线和闭环完成等决策信标，但这些信标仍主要依赖胶囊文字和底色。玩家在当前路线宽栏里扫视时，可以再补一层无文字状态点，让“这条路线为什么被推荐/当前/改道/完成”先被图形扫到。

本轮决策：

- 新增“远航决策信标状态点”。
- `src/styles.css` 为 `.far-dispatch-branch-choice-summary-signal` 增加 `::before` 状态点，并按 focus / stable / active / shift / completed 使用不同颜色。
- 当前路线内的决策信标播放 `farDispatchSummarySignalBeacon` 轻量补光，并纳入 `prefers-reduced-motion: reduce`。
- `tests/game.test.js` 覆盖状态点、状态色、当前路线动效、关键帧和降动效绑定。
- 该改动只增强远航路线对照条扫视性和测试，不改变远航收益、指令冷却、闭环、连段、投送、星图航段、升级价格、存档字段、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-05 20:17 CST 当前 5 个 open feedback issue、0 个 open bug issue；#6 作为本轮主处理对象。
- 20M 后路线对照里的 `far-dispatch-branch-choice-summary-signal` 带状态点；当前路线信标播放轻量补光，降低动效偏好下关闭动画。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 127 项。
- 构建产物已确认 `dist/src/styles.css` 包含 `far-dispatch-branch-choice-summary-signal::before` 与 `farDispatchSummarySignalBeacon`。
- 代码提交已创建并推送：`98d7b18 feat: add far decision signal status dots`。
- 发布验证已通过：GitHub Pages workflow `25376280716` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页和 `src/styles.css` 均返回 HTTP 200，线上产物已确认包含 `far-dispatch-branch-choice-summary-signal::before` 与 `farDispatchSummarySignalBeacon`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航决策信标状态点、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4379194014`。
- 回复后同步 GitHub Issues：2026-05-05 20:28 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue；#6 更新时间为 `2026-05-05T12:27:58Z`。
...
当前最大问题：反馈表单标题、字段、提交按钮和状态回执已有短标，但反馈表单整体仍缺少一块真正的本地插画资产。玩家进入侧栏反馈入口时，仍主要看到纵向文字字段和控件。

本轮决策：

- 新增“反馈表单本地插画”。
- `src/assets/feedback-visual.svg` 提供随仓库发布的本地 SVG 插画，表达反馈草稿、评分轨和 Issue 投递路径。
- `index.html` 在反馈标题下方引用该插画；`src/styles.css` 增加稳定的 `feedback-scene-image` 展示尺寸。
- `tests/game.test.js` 覆盖静态首页、样式绑定和 SVG 资产存在。
- 该改动只调整反馈表单视觉展示和测试，不新增可见说明文字，不改变反馈类型、评分、内容输入、校验、Issue 草稿、反馈快照、localStorage 事件、玩法数值、星图、航线指令、远航调度或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-05 19:32 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4 作为本轮主处理对象。
- 反馈表单标题下方显示 `feedback-scene-image`，引用 `src/assets/feedback-visual.svg`，并保持 `aria-labelledby="feedback-title"`、字段绑定和提交行为不变。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test`、`npm run build`；测试数 127 项。
...
- 代码提交已创建并推送：`9826ee5 feat: add feedback form visual asset`。
- 发布验证已通过：GitHub Pages workflow `25374076553` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页、`src/styles.css` 和 `src/assets/feedback-visual.svg` 均返回 HTTP 200，线上产物已确认包含 `feedback-scene-image`、`feedback-visual.svg` 与 `反馈草稿视觉插画`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明反馈表单本地插画、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4378850338`。
- 回复后同步 GitHub Issues：2026-05-05 19:39 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue；#4 更新时间为 `2026-05-05T11:39:09Z`。
...
- 代码提交已创建并推送：`0033340 feat: add feedback status visual marker`。
- 发布验证已通过：GitHub Pages workflow `25372713048` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `index.html` 和 `src/styles.css` 已确认包含 `feedbackStatus`、`.feedback-status:not(:empty)` 与 `grid-template-columns: 20px minmax(0, 1fr)`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明反馈状态回执视觉短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4378655303`。
- 回复后同步 GitHub Issues：2026-05-05 19:07 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue；#4 更新时间为 `2026-05-05T11:07:24Z`。
...
- 代码提交已创建并推送：`02560aa feat: add feedback submit visual marker`。
- 发布验证已通过：GitHub Pages workflow `25372097988` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `index.html` 和 `src/styles.css` 已确认包含 `feedback-submit-icon` 与 `grid-template-columns: 22px auto`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明反馈提交按钮发送短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4378570957`。
- 回复后同步 GitHub Issues：2026-05-05 18:53 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue；#4 更新时间为 `2026-05-05T10:53:40Z`。
...
- GitHub Issues 已同步：2026-05-05 18:00 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4 作为本轮主处理对象。回复后于 2026-05-05 18:09 CST 再次同步，当前仍为 5 个 open feedback issue、0 个 open bug issue；#4 更新时间为 2026-05-05T10:09:31Z。
...
- 发布验证已通过：GitHub Pages workflow `25370211866` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.offline-notice::before`、`.action-notice::before` 与 `grid-template-columns: 30px minmax(0, 1fr)`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明主操作行动提示视觉短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4378307697`。
...
- 代码提交已创建并推送：`0643441 feat: add goal strip visual badge`。
- 发布验证已通过：GitHub Pages workflow `25369522833` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `index.html` 和 `src/styles.css` 已确认包含 `goal-visual` 与 `goal-copy`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明当前目标条视觉徽记、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4378197523`。
- 回复后同步 GitHub Issues：2026-05-05 17:53 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue；#4 更新时间为 2026-05-05T09:53:09Z。
- 钉钉通知未发送：2026-05-05 17:56 CST 运行环境未提供 `DING` / `DINGTALK` / `WEBHOOK` / `ROBOT` 相关变量名，当前目录和 `/home/jassy/glm` 两层内未发现 `.env*` 文件；未将 webhook 写入仓库。
...
- 发布验证已通过：GitHub Pages workflow `25368025510` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/feedback.js` 已确认包含 `farRouteLoopStreak` 与 `- 远航连段：`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航连段反馈快照短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377991920`。
...
- 发布验证已通过：GitHub Pages workflow `25367356111` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 已确认包含 `far-dispatch-loop-streak` 和 `has-loop-streak`，线上 `src/styles.css` 已确认包含 `.far-dispatch-loop-streak`、`.far-dispatch.has-loop-streak` 和 `farDispatchLoopStreakBeacon`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航连段主调度条信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377874170`。
...
- 发布验证已通过：GitHub Pages workflow `25366738408` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/game.js` 已确认包含 `dispatchBadgeText: project.dispatchBadgeText`，线上 `src/app.js` 已确认包含 `project-current-dispatch` 和 `is-dispatch-active`，线上 `src/styles.css` 已确认包含 `.project-current-track .project-current-dispatch`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航连段当前航段视觉卡短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377786318`。
...
- GitHub Issues 已同步：2026-05-05 16:18 CST 当前 5 个 open feedback issue、0 个 open bug issue；#6 作为本轮主处理对象。
...
- 发布验证已通过：GitHub Pages workflow `25365301558` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/game.js` 已确认包含 `dispatch.loopStreakText` 与去重逻辑。
- 已回复 GitHub Issue #6；初次评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377567960`。因初次评论中的反引号文本被命令行展开为空白，已追加更正评论明确星图总览显示 `连段 X/3`，更正评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377585377`。issue 保持 open，等待复测。
...
- 发布验证已通过：GitHub Pages workflow `25364636860` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 已确认 `dispatchLoopStreak` 紧跟 `dispatchRouteStep`，线上 `src/styles.css` 已确认包含 `directiveDispatchLoopStreakBeacon` 和 `.directive-button.is-dispatch-route-step .directive-dispatch-loop-streak`。
- 已回复 GitHub Issue #6，说明远航连段按钮徽标信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377475682`。
...
- 发布验证已通过：提交 `66b924e` 已推送到 `origin/main`；GitHub Pages workflow `25364043302` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 已确认包含 `has-loop-streak`，线上 `src/styles.css` 已确认包含 `farDispatchSummaryLoopStreakResultGlow` 和 `has-loop-streak`。
- 已回复 GitHub Issue #6，说明远航连段回航结果信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377395032`。
...
- 发布验证已通过：提交 `ded077f` 已推送到 `origin/main`；GitHub Pages workflow `25363471584` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/game.js` 已确认包含 `routeLoopStreakText` 和连段短标逻辑。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明路线对照条连段回航短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377308038`。
...
- 发布验证已通过：提交 `80db087` 已推送到 `origin/main`；GitHub Pages workflow `25362720242` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/game.js` 已确认包含 `state?.farRouteLoopStreak`、“闭环复盘”和“连段”关键标记。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航连段闭环复盘、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377188747`。
...
- 发布验证已通过：提交 `51e8f20` 已推送到 `origin/main`；GitHub Pages workflow `25362112691` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/game.js` 已确认包含 `远航连段`、`farRouteLoopStreak` 和 `dispatchLoopStreakReward`，线上 `src/app.js` 已确认包含 `dispatchLoopStreakReward` 和 `directive-dispatch-loop-streak`，线上 `src/styles.css` 已确认包含 `directive-dispatch-loop-streak`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航闭环连段、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4377111946`。
...
- 发布验证已通过：提交 `b014a7d` 已推送到 `origin/main`；GitHub Pages workflow `25361305407` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `directiveDispatchRouteStatusBeacon` 和 `.directive-button.is-dispatch-route-step .directive-status::before`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航路线步骤按钮状态锚点、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4376977252`。
...
当前最大问题：上一轮已经让 20M 后当前远航路线步骤对应的航线指令按钮获得整按钮级高亮，但它仍主要是静态强调。玩家在目标 -> 分支/续航 -> 回目标连续操作时，当前按钮可以用更轻的动态信标和远航闭环微图、步骤卡、路线对照条保持同层节奏。

本轮决策：

- 新增“远航路线步骤按钮行动信标”。
- `src/styles.css` 让 `.directive-button.is-dispatch-route-step` 播放 `directiveDispatchRouteStepBeacon` 轻量背景扫光和饱和度变化。
- 在 `prefers-reduced-motion: reduce` 中关闭该行动信标动画。
- `tests/game.test.js` 增加静态断言，覆盖按钮行动信标动画、keyframes 和降低动效兜底。
- 该改动只调整当前远航路线步骤对应指令按钮的展示和测试，不新增可见文字、不新增收益、不新增存档字段，不改变指令收益、冷却、连携窗口、星图路线、项目奖励、升级价格、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-05 14:01 CST 当前 5 个 open feedback issue、0 个 open bug issue；#6 作为本轮主处理对象。
- `src/styles.css` 包含 `directiveDispatchRouteStepBeacon 1450ms ease-in-out infinite`、`@keyframes directiveDispatchRouteStepBeacon` 和降低动效兜底。
- `tests/game.test.js` 覆盖远航路线步骤按钮行动信标静态绑定。
- 本地完整验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
...
- 发布验证已通过：提交 `511f263` 已推送到 `origin/main`；GitHub Pages workflow `25360748185` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `directiveDispatchRouteStepBeacon` 和 `.directive-button.is-dispatch-route-step`。
- 已回复 GitHub Issue #6，说明远航路线步骤按钮行动信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4376904008`。
...
- 发布验证已通过：提交 `c6f4655` 已推送到 `origin/main`；GitHub Pages workflow `25360280007` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 和 `src/styles.css` 已确认包含 `is-dispatch-route-step` 与对应按钮高亮样式。
- 已回复 GitHub Issue #6，说明远航路线步骤按钮信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4376845333`。
...
- 发布验证已通过：提交 `f3923e5` 已推送到 `origin/main`；GitHub Pages workflow `25359776604` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `coreStableReadoutIdleBeacon` 和降低动效兜底。
- 已回复 GitHub Issue #5，说明点火过载状态读数待机信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4376780107`。
...
当前最大问题：点火按钮已经有完整点击后反馈链、普通待机信标链、鼠标 hover 预热投光和键盘焦点预热投光；但 0 连击普通待机时，下方 `连击 0` 读数仍是静态胶囊。玩家还没按下之前，连击读数可以和按钮本体、下一击预告、连击轨、过载倒计时一起参与点击前邀请。

本轮决策：

- 新增“点火连击读数待机信标”。
- `src/styles.css` 为 `.core-button:not(.is-combo-charging):not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) ~ .combo-line #comboValue` 增加 `coreComboReadoutIdleBeacon` 轻量呼吸。
- 在 `prefers-reduced-motion: reduce` 中关闭该待机信标动画。
- `tests/game.test.js` 增加静态断言，覆盖连击读数待机信标选择器、keyframes 和降低动效兜底。
- 该改动只调整 0 连击普通待机时的连击读数展示和测试，不新增可见文字、不新增收益、不新增存档字段，不改变点击收益、连击窗口、过载奖励、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 09:51 CST 当前 5 个 open feedback issue、0 个 open bug issue；#5 作为本轮主处理对象。
- `src/styles.css` 包含 `coreComboReadoutIdleBeacon 2600ms ease-in-out infinite`、`@keyframes coreComboReadoutIdleBeacon` 和降低动效兜底。
...
- 发布验证已通过：提交 `6f1a360` 已推送到 `origin/main`；GitHub Pages workflow `25240693569` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-button:focus-visible`、`coreArtHoverPreheat` 和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火键盘焦点预热投光、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362572319`。
...
- 发布验证已通过：提交 `18419cc` 已推送到 `origin/main`；GitHub Pages workflow `25240353134` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-art`、`coreArtHoverPreheat` 和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火悬停预热投光、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362534650`。
...
- 发布验证已通过：提交 `8158b02` 已推送到 `origin/main`；GitHub Pages workflow `25239639815` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-feedback-layer`、`coreFeedbackIdleBeacon` 和降低动效兜底。
- 已回复 GitHub Issue #5，说明点火粒子层待机信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362456794`。
...
当前最大问题：点火按钮已经有待机邀请光环、下一击预告待机信标、中心标签待机信标、蓄能外弧待机信标、连击轨下一格待机信标、倒计时徽标待机信标、蓄能轨待机信标，以及完整点击后反馈链路。阶段光环会随连击进度升温，并在命中、过载前一击和过载命中时反馈明显，但 0 连击普通待机时仍偏静态；玩家还没按下之前，内层阶段环可以更主动提示“这里会从第一击开始蓄能”。

本轮决策：

- 新增“点火阶段光环待机信标”。
- `src/styles.css` 为 `.core-button:not(.is-combo-charging):not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-stage-aura` 增加 `coreStageIdleBeacon` 轻量呼吸。
- 在 `prefers-reduced-motion: reduce` 中关闭该待机信标动画。
- `tests/game.test.js` 增加静态断言，覆盖阶段光环待机信标样式、keyframes 和降低动效兜底。
- 该改动只调整点火按钮阶段光环展示层和测试，不新增可见文字、不新增收益、不新增存档字段，不改变点击收益、连击窗口、过载奖励、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 08:30 CST 当前 5 个 open feedback issue、0 个 open bug issue；#5 作为本轮主处理对象。
- `src/styles.css` 包含 `.core-stage-aura` 待机选择器、`coreStageIdleBeacon 2800ms ease-in-out infinite`、`@keyframes coreStageIdleBeacon` 和降低动效兜底。
- `tests/game.test.js` 覆盖点火阶段光环待机信标静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认 `dist/src/styles.css` 包含 `coreStageIdleBeacon 2800ms ease-in-out infinite`、`@keyframes coreStageIdleBeacon` 和降低动效兜底。
- 发布验证已通过：提交 `e8bb90a` 已推送到 `origin/main`；GitHub Pages workflow `25239290893` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-stage-aura`、`coreStageIdleBeacon` 和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火阶段光环待机信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362389655`。
...
## 2026-05-02 Product decision：点火蓄能轨待机信标

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 08:14 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有 open bug；#5 仍是最近更新反馈，继续处理“点火按钮太薄弱，增加点击反馈、特效和点击欲望”。

当前最大问题：点火按钮已经有待机邀请光环、下一击预告待机信标、中心标签待机信标、蓄能外弧待机信标、连击轨下一格待机信标、倒计时徽标待机信标，以及完整点击后反馈链路。按钮环形蓄能轨会随连击点亮，并在普通命中和过载命中时反馈明显，但 0 连击普通待机时仍偏静态；玩家还没按下之前，环形蓄能轨可以更主动提示“这条蓄能轨会被下一击推进”。

本轮决策：

- 新增“点火蓄能轨待机信标”。
- `src/styles.css` 为 `.core-button:not(.is-combo-charging):not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-charge-ring` 增加 `coreChargeRingIdleBeacon` 轻量呼吸。
- 在 `prefers-reduced-motion: reduce` 中关闭该待机信标动画。
- `tests/game.test.js` 增加静态断言，覆盖蓄能轨待机信标样式、keyframes 和降低动效兜底。
- 该改动只调整点火按钮环形蓄能轨展示层和测试，不新增可见文字、不新增收益、不新增存档字段，不改变点击收益、连击窗口、过载奖励、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 08:14 CST 当前 5 个 open feedback issue、0 个 open bug issue；#5 作为本轮主处理对象。
- `src/styles.css` 包含 `.core-button:not(.is-combo-charging):not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-charge-ring`、`coreChargeRingIdleBeacon 2600ms ease-in-out infinite`、`@keyframes coreChargeRingIdleBeacon` 和降低动效兜底。
- `tests/game.test.js` 覆盖点火蓄能轨待机信标静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
...
- 发布验证已通过：提交 `2731976` 已推送到 `origin/main`；GitHub Pages workflow `25238924337` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-charge-ring`、`coreChargeRingIdleBeacon` 和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火蓄能轨待机信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362311687`。
...
## 2026-05-02 Product decision：点火倒计时徽标待机信标

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 07:59 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有 open bug；#5 仍是最近更新反馈，继续处理“点火按钮太薄弱，增加点击反馈、特效和点击欲望”。

当前最大问题：点火按钮已经有待机邀请光环、下一击预告待机信标、中心标签待机信标、蓄能外弧待机信标、连击轨下一格待机信标，以及完整点击后反馈链路。按钮左上过载倒计时徽标会显示还差几次过载，并在命中和过载前一击时反馈明显，但普通待机时仍偏静态；玩家还没按下之前，“再点几次触发过载”的小目标可以更主动参与点击前邀请。

本轮决策：

- 新增“点火倒计时徽标待机信标”。
- `src/styles.css` 为 `.core-button:not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-overload-badge` 增加 `coreBadgeIdleBeacon` 轻量呼吸。
- 在 `prefers-reduced-motion: reduce` 中关闭该待机信标动画。
- `tests/game.test.js` 增加静态断言，覆盖倒计时徽标待机信标样式、keyframes 和降低动效兜底。
- 该改动只调整点火按钮过载倒计时徽标展示层和测试，不新增可见文字、不新增收益、不新增存档字段，不改变点击收益、连击窗口、过载奖励、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 07:59 CST 当前 5 个 open feedback issue、0 个 open bug issue；#5 作为本轮主处理对象。
- `src/styles.css` 包含 `.core-button:not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-overload-badge`、`coreBadgeIdleBeacon 2400ms ease-in-out infinite`、`@keyframes coreBadgeIdleBeacon` 和降低动效兜底。
...
阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 07:43 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有 open bug；#5 仍是最近更新反馈，继续处理“点火按钮太薄弱，增加点击反馈、特效和点击欲望”。

当前最大问题：点火按钮已经有待机邀请光环、下一击预告待机信标、中心标签待机信标、蓄能外弧待机信标、按住反冲、落点热区、命中闪光、涟漪、火花束、收益浮层、蓄能轨、连击轨、星核裂纹、读数亮闪、音效和触感。普通待机时，外圈、奖励预告、中心标签和外弧已经在邀请点击，但 8 格连击轨里的下一格仍只是静态白点；玩家还没点击前，“这一击会推进到哪一格”的提示可以更主动。

本轮决策：

- 新增“点火连击轨下一格待机信标”。
- `src/styles.css` 为 `.core-button:not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-combo-dot.is-next` 增加 `coreComboNextIdleBeacon` 轻量呼吸。
- 在 `prefers-reduced-motion: reduce` 中关闭该待机信标动画。
- `tests/game.test.js` 增加静态断言，覆盖下一格待机信标样式、keyframes 和降低动效兜底。
- 该改动只调整点火按钮 8 格连击轨展示层和测试，不新增可见文字、不新增收益、不新增存档字段，不改变点击收益、连击窗口、过载奖励、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 07:43 CST 当前 5 个 open feedback issue、0 个 open bug issue；#5 作为本轮主处理对象。
- `src/styles.css` 包含 `.core-button:not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-combo-dot.is-next`、`coreComboNextIdleBeacon 1800ms ease-in-out infinite`、`@keyframes coreComboNextIdleBeacon` 和降低动效兜底。
- `tests/game.test.js` 覆盖点火连击轨下一格待机信标静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
...
- 发布验证已通过：提交 `1f4f612` 已推送到 `origin/main`；GitHub Pages workflow `25238179705` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-button:not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-combo-dot.is-next`、`coreComboNextIdleBeacon` 和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火连击轨下一格待机信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362195903`。
...
- 发布验证已通过：提交 `1544666` 已推送到 `origin/main`；GitHub Pages workflow `25237801283` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-button:not(.is-combo-charging):not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-surge-orbit`、`coreSurgeIdleBeacon` 和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火蓄能外弧待机信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362155703`。
...
当前最大问题：点火按钮已经有待机邀请光环、下一击预告待机信标、按住反冲、落点热区、命中闪光、涟漪、火花束、收益浮层、蓄能轨、连击轨、读数亮闪、音效和触感。普通待机时，按钮外圈和下方下一击预告已经在邀请点击，但中心“点火”标签仍是静态白色圆心；玩家第一眼扫到可点击目标时，核心操作词本身还没有参与点击前节奏。

本轮决策：

- 新增“点火中心标签待机信标”。
- `src/styles.css` 为 `.core-button:not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-label` 增加 `coreLabelIdleBeacon` 轻量提亮，并在 hover 时增强中心标签光晕。
- 在 `prefers-reduced-motion: reduce` 中关闭该待机信标动画。
- `tests/game.test.js` 增加静态断言，覆盖待机中心标签样式、hover 样式、keyframes 和降低动效兜底。
- 该改动只调整点火按钮中心标签展示层和测试，不新增可见文字、不新增收益、不新增存档字段，不改变点击收益、连击窗口、过载奖励、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 07:15 CST 当前 5 个 open feedback issue、0 个 open bug issue；#5 作为本轮主处理对象。
- `src/styles.css` 包含 `.core-button:not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-label`、`coreLabelIdleBeacon 2400ms ease-in-out infinite`、hover 提亮样式、`@keyframes coreLabelIdleBeacon` 和降低动效兜底。
- `tests/game.test.js` 覆盖点火中心标签待机信标静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
...
- 发布验证已通过：提交 `2f25039` 已推送到 `origin/main`；GitHub Pages workflow `25237401109` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-button:not(.is-pulsing):not(.is-pressing):not(.is-overload-ready):not(.is-overload-hit) .core-label`、`coreLabelIdleBeacon` 和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火中心标签待机信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362118542`。
...
- 发布验证已通过：提交 `bf8081b` 已推送到 `origin/main`；GitHub Pages workflow `25237056485` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-button:not(.is-pulsing):not(.is-pressing) + .core-reward-hint:not(.is-overload-ready):not(.is-overload-hit)`、`coreRewardHintIdleBeacon` 和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火下一击预告待机信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362082915`。
...
当前最大问题：点火按钮已经有按住反冲、落点热区、命中闪光、涟漪、火花束、收益浮层、蓄能轨、连击轨、中心标签、星核裂纹、读数亮闪、音效和触感，但这些大多发生在按下或命中之后。玩家进入页面、还没有点击时，按钮外圈仍缺少一个轻量的“这里可以继续点”的待机邀请信号。

本轮决策：

- 新增“点火按钮待机邀请光环”。
- `src/styles.css` 为 `.core-button:not(.is-pulsing):not(.is-pressing)::after` 增加 `coreIdleInviteRing` 轻量外圈呼吸，并在 hover 时稍微加强边框。
- 在 `prefers-reduced-motion: reduce` 中关闭该待机光环动画。
- `tests/game.test.js` 增加静态断言，覆盖待机光环样式、keyframes 和降低动效兜底。
- 该改动只调整点火按钮展示层和测试，不新增可见文字、不新增收益、不新增存档字段，不改变点击收益、连击窗口、过载奖励、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 06:49 CST 当前 5 个 open feedback issue、0 个 open bug issue；#5 作为本轮主处理对象。
- `src/styles.css` 包含 `.core-button:not(.is-pulsing):not(.is-pressing)::after`、`coreIdleInviteRing 2200ms ease-in-out infinite`、`@keyframes coreIdleInviteRing` 和降低动效兜底。
- `tests/game.test.js` 覆盖点火按钮待机邀请光环静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
...
- 发布验证已通过：提交 `49ec7d0` 已推送到 `origin/main`；GitHub Pages workflow `25236657600` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.core-button:not(.is-pulsing):not(.is-pressing)::after`、`coreIdleInviteRing` 和降低动效兜底。
- 已回复 GitHub Issue #5，说明点火按钮待机邀请光环、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4362045488`。
...
## 2026-05-02 Product decision：远航闭环当前步骤正文锚点

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 06:37 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有 open bug；#4/#6 仍是主线反馈，继续指向远航调度需要更多图形化表达、后半段短循环需要更少文字反推。

当前最大问题：远航闭环插画、路线微图、进度轨、当前列、当前节点、方向信标、连接信标、当前步骤卡、当前步号、内侧竖线、当前收益行和当前短标题已经进入同一反馈节奏，但步骤卡正文状态行仍是普通白字。玩家从短标题扫到收益行时，还可以更快确认“当前这行正文就是正在执行的步骤描述”。

本轮决策：

- 新增“远航闭环当前步骤正文锚点”。
- `src/styles.css` 为 `.far-dispatch-loop-step.is-current span` 增加左侧短边、浅色底和当前文本色。
- `tests/game.test.js` 增加静态断言，覆盖当前步骤正文锚点样式。
- 该改动只调整远航闭环展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 06:37 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/styles.css` 包含 `.far-dispatch-loop-step.is-current span`、`border-left: 2px solid rgba(255, 211, 106, 0.5)` 和 `color: #fffbe8`。
- `tests/game.test.js` 覆盖远航闭环当前步骤正文锚点静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
...
- 发布验证已通过：提交 `429f556` 已推送到 `origin/main`；GitHub Pages workflow `25236292383` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.far-dispatch-loop-step.is-current span`、`border-left: 2px solid rgba(255, 211, 106, 0.5)` 和 `color: #fffbe8`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环当前步骤正文锚点、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4362006644`。
- 已回复 GitHub Issue #6，说明该锚点让目标 -> 分支/续航 -> 回目标短循环中的当前步骤正文更容易对应到实际按钮节奏；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4362007563`。
...
- 发布验证已通过：提交 `5e425fd` 已推送到 `origin/main`；GitHub Pages workflow `25235917443` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `.far-dispatch-loop-step.is-current strong`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环当前步骤标题锚点、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361941052`。
- 已回复 GitHub Issue #6，说明该锚点让目标 -> 分支/续航 -> 回目标短循环中的当前步骤标题更容易对应到实际按钮节奏；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361941959`。
...
- 发布验证已通过：提交 `b905f93` 已推送到 `origin/main`；GitHub Pages workflow `25235471380` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `farDispatchLoopCurrentPointColumnGlow`、当前微图列补光样式和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环当前微图列补光、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361895610`。
- 已回复 GitHub Issue #6，说明该补光让目标 -> 分支/续航 -> 回目标短循环中的当前步骤与回报落点更容易对应；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361895622`。
...
当前最大问题：远航闭环插画、路线微图、进度轨、当前节点、方向信标、连接信标、当前步骤卡、微图当前收益短标、步骤卡当前收益行和当前步号已经进入同一反馈节奏，但步骤卡左侧连接步号和收益行的内侧竖线仍是静态状态。玩家从 1/2/3 步号扫到本步收益时，这条最短视觉连线还可以更直接地确认“这张卡正在执行”。

本轮决策：

- 新增“远航闭环步骤卡当前竖线补光”。
- `src/styles.css` 为 `.far-dispatch.is-active .far-dispatch-loop-step.is-current::after` 增加 `farDispatchLoopCurrentStepRailGlow` 轻量补光。
- 在 `prefers-reduced-motion: reduce` 中关闭该竖线动效。
- `tests/game.test.js` 增加静态断言，覆盖 active 当前步骤卡竖线补光、keyframes 和降低动效兜底。
- 该改动只调整远航闭环展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 05:54 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/styles.css` 包含 `.far-dispatch.is-active .far-dispatch-loop-step.is-current::after`、`farDispatchLoopCurrentStepRailGlow` 和降低动效兜底。
- `tests/game.test.js` 覆盖远航闭环步骤卡当前竖线补光静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认 `dist/src/styles.css` 包含 `farDispatchLoopCurrentStepRailGlow`、当前步骤卡竖线补光样式和降低动效兜底。
- 发布验证已通过：提交 `d3c9dc1` 已推送到 `origin/main`；GitHub Pages workflow `25234989096` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `farDispatchLoopCurrentStepRailGlow`、当前步骤卡竖线补光样式和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环步骤卡当前竖线补光、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361843066`。
- 已回复 GitHub Issue #6，说明该补光让目标 -> 分支 -> 回目标短循环中的当前步骤卡执行状态更容易确认；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361843862`。
...
## 2026-05-02 Product decision：远航闭环步骤卡当前步号补光

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 05:43 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有 open bug；#4/#6 仍是主线反馈，继续指向远航调度需要更多图形化表达、后半段短循环需要更少文字反推。

当前最大问题：远航闭环插画、路线微图、进度轨、当前节点、方向信标、连接信标、当前步骤卡、微图当前收益短标和步骤卡当前收益行已经进入同一反馈节奏，但步骤卡左侧的 1/2/3 当前步号仍只是静态高亮。玩家从上方微图扫到下方卡片时，当前步骤编号这个最短图形锚点还可以更直接地确认“现在执行第几步”。

本轮决策：

- 新增“远航闭环步骤卡当前步号补光”。
- `src/styles.css` 为 `.far-dispatch.is-active .far-dispatch-loop-step.is-current::before` 增加 `farDispatchLoopCurrentStepBadgeGlow` 轻量补光。
- 在 `prefers-reduced-motion: reduce` 中关闭该步号动效。
- `tests/game.test.js` 增加静态断言，覆盖 active 当前步骤卡步号补光、keyframes 和降低动效兜底。
- 该改动只调整远航闭环展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 05:43 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/styles.css` 包含 `.far-dispatch.is-active .far-dispatch-loop-step.is-current::before`、`farDispatchLoopCurrentStepBadgeGlow` 和降低动效兜底。
- `tests/game.test.js` 覆盖远航闭环步骤卡当前步号补光静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认 `dist/src/styles.css` 包含 `farDispatchLoopCurrentStepBadgeGlow`、当前步骤卡步号补光样式和降低动效兜底。
- 发布验证已通过：提交 `f47f1a1` 已推送到 `origin/main`；GitHub Pages workflow `25234591348` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `farDispatchLoopCurrentStepBadgeGlow`、当前步骤卡步号补光样式和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环步骤卡当前步号补光、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361800723`。
- 已回复 GitHub Issue #6，说明该补光让目标 -> 分支 -> 回目标短循环的当前步骤编号更容易确认；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361801589`。
...
- 发布验证已通过：提交 `470148e` 已推送到 `origin/main`；GitHub Pages workflow `25234143073` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `farDispatchStepRewardCurrentGlow`、当前步骤卡收益行补光样式和降低动效兜底。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
...
当前最大问题：远航闭环插画、进度轨、当前节点、方向信标、连接信标和步骤卡已经进入同一反馈节奏，但路线微图节点下方的 `校准` / `分支` / `续航` / `闭环` 收益短标当前仍是静态状态。玩家在目标 -> 分支 -> 回目标连续操作时，当前步骤的回报落点还可以更直接地随当前态被扫到。

本轮决策：

- 新增“远航闭环当前收益短标补光”。
- `src/styles.css` 为 `.far-dispatch.is-active .far-dispatch-loop-visual-reward.is-current` 增加 `farDispatchLoopCurrentRewardGlow` 轻量补光。
- 在 `prefers-reduced-motion: reduce` 中关闭该收益短标动效。
- `tests/game.test.js` 增加静态断言，覆盖 active 当前收益短标动画、keyframes 和降低动效兜底。
- 该改动只调整远航闭环展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 05:14 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/styles.css` 包含 `.far-dispatch.is-active .far-dispatch-loop-visual-reward.is-current`、`farDispatchLoopCurrentRewardGlow` 和降低动效兜底。
- `tests/game.test.js` 覆盖远航闭环当前收益短标补光静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认 `dist/src/styles.css` 包含 `farDispatchLoopCurrentRewardGlow`、当前收益短标补光样式和降低动效兜底。
- 发布验证已通过：提交 `6d0bb1e` 已推送到 `origin/main`；GitHub Pages workflow `25233674862` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `farDispatchLoopCurrentRewardGlow` 和当前收益短标补光样式。
- 已回复 GitHub Issue #4，说明远航闭环当前收益短标补光、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361705628`。
- 已回复 GitHub Issue #6，说明当前收益短标补光让“目标 -> 分支 -> 回目标”的当前回报落点更容易确认；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361705630`。
...
## 2026-05-02 Product decision：远航闭环进度轨活跃态光扫

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 04:58 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有 open bug；#4/#6 仍是主线反馈，继续指向远航调度需要更图形化、后半段短循环需要更容易扫视。

当前最大问题：远航调度区已有闭环插画、路线微图、当前节点脉冲、当前方向信标、微图到步骤卡连接信标和当前步骤卡信标，但路线微图的进度轨本体仍是静态填充。玩家在 20M 后连续执行目标 -> 分支 -> 回目标时，轨道推进本身还缺少和当前节点/方向/连接层一致的轻量动态确认。

本轮决策：

- 新增“远航闭环进度轨活跃态光扫”。
- `src/styles.css` 为 `.far-dispatch.is-active .far-dispatch-loop-visual::after` 增加 `farDispatchLoopVisualTrackSweep` 轻量光扫动画。
- 在 `prefers-reduced-motion: reduce` 中关闭该进度轨动效。
- `tests/game.test.js` 增加静态断言，覆盖 active 进度轨动画、keyframes 和降低动效兜底。
- 该改动只调整远航闭环展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 04:58 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/styles.css` 包含 `.far-dispatch.is-active .far-dispatch-loop-visual::after`、`farDispatchLoopVisualTrackSweep` 和降低动效兜底。
...
- 发布验证已通过：提交 `7eba680` 已推送到 `origin/main`；GitHub Pages workflow `25232623485` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上首页和 `src/styles.css` 已确认包含远航调度插画与 `farDispatchSceneImagePulse`。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航调度闭环插画 active 状态投光、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361588500`。
- 已回复 GitHub Issue #6，说明远航调度闭环插画 active 状态投光、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361589408`。
...
- 发布验证已通过：提交 `40a5154` 已推送到 `origin/main`；GitHub Pages workflow `25232062394` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上首页、`src/app.js` 和 `src/assets/far-dispatch-visual.svg` 已确认包含动态插画修复。workflow 继续给出 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航调度闭环插画现在会在运行时动态渲染后继续保留；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361524700`。
- 已回复 GitHub Issue #6，说明该修复让“目标 -> 分支 -> 回目标”的图形锚点在真实运行态持续可见；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361524737`。
...
当前最大问题：远航调度已经有路线微图、对照条、步骤卡、连接信标和当前脉冲，但远航调度区本体仍缺少一张概览型图片。#4 明确要求“生成一些图片”，当前远航闭环的多层短标虽然降低了文字反推，但还没有把“目标 -> 分支 -> 回目标”的整体航线关系先用一张图建立情绪和空间锚点。

本轮决策：

- 新增本地 SVG 资产 `src/assets/far-dispatch-visual.svg`，用三步闭环、协同航线、绕行航线和远航飞行器表达远航调度。
- `index.html` 在 `#farDispatch` 内加入 `.far-dispatch-scene-image`，随静态构建发布，不依赖远端图片。
- `src/styles.css` 为远航插画增加默认、active 和 locked 三种展示状态；未解锁时降低饱和度，解锁后增强边框和轻量投光。
- `tests/game.test.js` 增加静态断言，覆盖首页引用、SVG 语义和样式绑定。
- 该改动只新增远航调度展示资产和样式，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 04:15 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `index.html` 包含 `.far-dispatch-scene-image` 和 `src/assets/far-dispatch-visual.svg`。
- `src/assets/far-dispatch-visual.svg` 包含 `farDispatchVisual`、标题和描述。
- `src/styles.css` 包含 `.far-dispatch-scene-image`、`.far-dispatch.is-active .far-dispatch-scene-image` 和 `.far-dispatch.is-locked .far-dispatch-scene-image`。
...
- 发布验证已通过：提交 `1b98512` 已推送到 `origin/main`；GitHub Pages workflow `25230970830` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `farDispatchLoopCurrentLinkPulse`。
- 已回复 GitHub Issue #4，说明远航闭环当前连接信标脉冲、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361393475`。
- 已回复 GitHub Issue #6，说明远航闭环当前连接信标脉冲、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361394799`。
...
当前最大问题：远航闭环路线微图已经有 1/2/3 节点、方向箭头、当前节点/方向信标和收益短标；下方步骤卡也有步号锚点、收益色带和当前步骤卡信标。但上方微图和下方步骤卡之间仍靠同列位置自行对应，缺少一个无文字的视觉连接层。

本轮决策：

- 新增“远航闭环微图到步骤卡连接信标”。
- `src/app.js` 在 `renderFarDispatchLoopTrack` 中从现有 `dispatch.loopSteps` 派生 3 个 `.far-dispatch-loop-link`，按步骤状态和收益类型复用 `is-current` / `is-completed` / `is-target` / `is-branch` / `is-relay` / `is-return` class。
- `src/styles.css` 给连接信标增加竖向线和小节点，并按目标、分支/续航、回目标、当前和完成状态着色。
- `tests/game.test.js` 增加静态断言，覆盖连接信标 DOM 绑定、无障碍隐藏和样式选择器。
- 该改动只调整远航闭环展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 03:42 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/app.js` 包含 `far-dispatch-loop-link`、`stepLinks` 和 `track.append(visual, ...stepLinks, ...stepItems)`。
- `src/styles.css` 包含 `.far-dispatch-loop-link`、`::before`、`::after`、目标/分支/续航/回目标状态和当前/完成状态样式。
- `tests/game.test.js` 覆盖远航闭环连接信标静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认包含 `far-dispatch-loop-link` 代码和样式。
- 发布验证已通过：提交 `c5f0637` 已推送到 `origin/main`；GitHub Pages workflow `25230411163` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 和 `src/styles.css` 已确认包含 `far-dispatch-loop-link`。
- 已回复 GitHub Issue #4，说明远航闭环微图到步骤卡连接信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361313495`。
- 已回复 GitHub Issue #6，说明远航闭环微图到步骤卡连接信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361314418`。
...
- 本轮未新增外部网页调研；依据来自真实 GitHub 反馈 #4/#6，以及当前远航闭环微图与步骤卡仍需列位置对应的复盘。

## 2026-05-02 Product decision：远航闭环当前步骤卡信标

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 03:24 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有 open bug；#4/#6 仍是主线反馈，继续指向后半段远航调度需要更多图形化表达并减少主操作区文字反推。

当前最大问题：远航闭环路线微图已经具备当前节点脉冲、当前方向信标、收益短标；步骤卡也有 1/2/3 步号和收益色带。但三张步骤卡本体仍主要靠静态背景区分当前态，玩家从微图扫到下方执行卡时，当前步骤卡还缺少一个不增加文字的轻量动作锚点。

本轮决策：

- 新增“远航闭环当前步骤卡信标”。
- `src/styles.css` 为 `.far-dispatch-loop-step.is-current` 增加轻量扫光、饱和度变化和 `farDispatchLoopCurrentStepGlow` 动画。
- 在 `prefers-reduced-motion: reduce` 中关闭当前步骤卡信标动画。
- `tests/game.test.js` 增加静态断言，覆盖当前步骤卡动画、keyframes 和降低动效兜底。
- 该改动只调整远航闭环步骤卡展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 03:24 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/styles.css` 包含 `.far-dispatch-loop-step.is-current`、`farDispatchLoopCurrentStepGlow` 和降低动效兜底。
- `tests/game.test.js` 覆盖远航闭环当前步骤卡信标静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认包含 `farDispatchLoopCurrentStepGlow`、`.far-dispatch-loop-step.is-current` 和降低动效兜底。
- 发布验证已通过：提交 `a26dc63` 已推送到 `origin/main`；GitHub Pages workflow `25229638994` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `farDispatchLoopCurrentStepGlow` 和 `.far-dispatch-loop-step.is-current`。
- 已回复 GitHub Issue #4，说明远航闭环当前步骤卡信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361226934`。
- 已回复 GitHub Issue #6，说明远航闭环当前步骤卡信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361228503`。
...
- 本轮未新增外部网页调研；依据来自真实 GitHub 反馈 #4/#6，以及当前远航闭环当前步骤卡仍偏静态的复盘。
...
- 发布验证已通过：提交 `c80ce19` 已推送到 `origin/main`；GitHub Pages workflow `25229061845` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 和 `src/styles.css` 已确认包含远航闭环步骤卡收益色带代码和样式。
- 已回复 GitHub Issue #4，说明远航闭环步骤卡收益色带、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361158046`。
- 已回复 GitHub Issue #6，说明远航闭环步骤卡收益色带、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361158048`。
...
当前最大问题：远航闭环路线微图已经具备 1/2/3 节点、方向箭头、当前方向信标、当前节点脉冲和收益短标，能表达进度、方向与回报落点；但下方三张步骤卡仍主要靠文字标题与上方微图对应。玩家看到微图第 2 步后，还需要从“协同/绕行”文字和卡片位置反推对应卡片，执行区仍有一次不必要的文字匹配。

本轮决策：

- 新增“远航闭环步骤卡步号锚点”。
- `src/app.js` 在 `renderFarDispatchLoopTrack` 中给每张 `.far-dispatch-loop-step` 追加 `data-step-label` 和目标/分支/续航/回目标类型 class，复用 `getFarDispatchLoopVisualRewardKind` 与上方微图保持一致。
- `src/styles.css` 为 `.far-dispatch-loop-step::before` / `::after` 增加 1/2/3 步号锚点和竖向状态线，并按目标、分支/续航、回目标以及当前/完成状态着色。
- `tests/game.test.js` 增加静态断言，覆盖步骤卡 class、`data-step-label`、伪元素、类型状态和当前/完成状态样式。
- 该改动只调整远航闭环步骤卡展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 02:55 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/app.js` 包含步骤卡 `data-step-label`、`far-dispatch-loop-step is-` 类型 class 和 `getFarDispatchLoopVisualRewardKind(step, index)` 复用。
- `src/styles.css` 包含 `.far-dispatch-loop-step::before`、`content: attr(data-step-label)`、`.far-dispatch-loop-step::after`、目标/分支/续航/回目标状态和当前/完成状态步号样式。
- `tests/game.test.js` 覆盖远航闭环步骤卡步号锚点静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认包含 `far-dispatch-loop-step is-`、`data-step-label` 和 `.far-dispatch-loop-step::before` 步号锚点样式。
- 发布验证已通过：提交 `3887c8c` 已推送到 `origin/main`；GitHub Pages workflow `25228543304` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 和 `src/styles.css` 已确认包含远航闭环步骤卡步号锚点代码和样式。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环步骤卡步号锚点、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361097829`。
- 已回复 GitHub Issue #6，说明远航闭环步骤卡步号锚点、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361097835`。
...
- 本轮未新增外部网页调研；依据来自真实 GitHub 反馈 #4/#6，以及当前远航闭环微图和步骤卡仍需对应成本的复盘。

## 2026-05-02 Product decision：远航闭环当前方向信标

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 02:37 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有 open bug；#4/#6 仍是主线反馈，继续指向后半段远航调度需要更多图形化表达并减少主操作区文字反推。

当前最大问题：远航闭环路线微图已经具备节点、方向箭头、推进填充、收益短标和当前节点脉冲，能表达当前格与每步回报；但当前推进方向仍主要靠静态黄色箭头确认。玩家在目标 -> 分支 -> 回目标的连续操作中，需要当前 1 -> 2 或 2 -> 3 的方向段也有轻量动作信号，而不是只看节点光标。

本轮决策：

- 新增“远航闭环当前方向信标”。
- `src/styles.css` 为 `.far-dispatch-loop-visual-arrow.is-current` 增加轻量方向信标，并新增 `farDispatchLoopCurrentArrowPulse` 动画。
- 在 `prefers-reduced-motion: reduce` 中关闭当前方向信标动画。
- `tests/game.test.js` 增加静态断言，覆盖当前方向箭头动画、keyframes 和降低动效兜底。
- 该改动只调整远航闭环微图展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 02:37 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/styles.css` 包含 `.far-dispatch-loop-visual-arrow.is-current`、`farDispatchLoopCurrentArrowPulse` 和降低动效兜底。
- `tests/game.test.js` 覆盖远航闭环当前方向信标静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认包含 `farDispatchLoopCurrentArrowPulse` 和当前方向信标样式。
- 发布验证已通过：提交 `5537b43` 已推送到 `origin/main`；GitHub Pages workflow `25227928985` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `farDispatchLoopCurrentArrowPulse` 和 `.far-dispatch-loop-visual-arrow.is-current`。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环当前方向信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4361026042`。
- 已回复 GitHub Issue #6，说明远航闭环当前方向信标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4361026048`。
...
- 本轮未新增外部网页调研；依据来自真实 GitHub 反馈 #4/#6，以及当前远航闭环微图仍需强化当前推进方向的复盘。

## 2026-05-02 Product decision：远航闭环当前节点脉冲

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 02:20 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有 open bug；#4/#6 仍是主线反馈，继续指向后半段远航调度需要更多图形化表达并减少主操作区文字反推。

当前最大问题：远航闭环路线微图已经具备 1/2/3 节点、方向箭头、推进填充和收益短标，能表达顺序、进度和回报落点；但当前执行节点仍主要依靠黄色节点与下方步骤卡共同确认，玩家在主操作区快速点击时还缺少一个不增加文字的“当前这格正在推进”视觉光标。

本轮决策：

- 新增“远航闭环当前节点脉冲”。
- `src/styles.css` 为 `.far-dispatch-loop-visual-node.is-current::after` 增加轻量脉冲光标，并新增 `farDispatchLoopCurrentPulse` 动画。
- 在 `prefers-reduced-motion: reduce` 中关闭当前节点脉冲动画。
- `tests/game.test.js` 增加静态断言，覆盖当前节点伪元素、keyframes 和降低动效兜底。
- 该改动只调整远航闭环微图展示层和测试，不新增可见说明文字、不新增收益、不新增存档字段，不改变点击收益、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度数值、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 02:20 CST 当前 5 个 open feedback issue、0 个 open bug issue；#4/#6 作为本轮主处理对象。
- `src/styles.css` 包含 `.far-dispatch-loop-visual-node.is-current::after`、`farDispatchLoopCurrentPulse` 和降低动效兜底。
...
- 发布验证已通过：提交 `0eef8e5` 已推送到 `origin/main`；GitHub Pages workflow `25226604785` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 和 `src/styles.css` 已确认包含远航闭环收益短标代码和样式。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环收益短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4360864356`。
- 已回复 GitHub Issue #6，说明远航闭环收益短标、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4360864334`。
...
- 发布验证已通过：提交 `9686f9b` 已推送到 `origin/main`；GitHub Pages workflow `25225962692` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 和 `src/styles.css` 已确认包含 `far-dispatch-loop-visual-arrow`、`getFarDispatchLoopVisualArrowState` 和箭头状态样式。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环方向箭头、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4360787110`。
- 已回复 GitHub Issue #6，说明远航闭环方向箭头、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4360787114`。
...
- 发布验证已通过：提交 `d2b6abf` 已推送到 `origin/main`；GitHub Pages workflow `25225334380` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 和 `src/styles.css` 已确认包含 `far-dispatch-loop-visual`、`--far-loop-visual-progress` 和节点状态样式。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #4，说明远航闭环路线微图、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4360713677`。
- 已回复 GitHub Issue #6，说明远航闭环路线微图、验证结果、Pages 部署和复测问题；issue 保持 open，评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4360713710`。
...
- 发布验证已通过：提交 `2f2df06` 已推送到 `origin/main`；GitHub Pages workflow `25224556618` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `coreButtonHitGlow`、`coreButtonOverloadGlow` 和降低动效兜底。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火按钮整体命中投光、验证结果、Pages 部署和复测问题；issue 保持 open，更新时间为 2026-05-01T17:20:13Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4360616868`。
- 回复后同步 GitHub Issues：2026-05-02 01:20 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue。
...
当前最大问题：点火按钮已经有按住反冲、落点反馈、收益浮层、顶部读数亮闪、连击轨光扫、倒计时徽标跳闪、星核裂纹闪亮和阶段光环补光。收益浮层已经跟随点击落点显示 `+X`，但它本身仍是单个胶囊浮层，缺少“这一击奖励从落点扩散出去”的短促奖励感。

本轮决策：

- 新增“点火收益浮层命中奖励光晕”。
- `src/styles.css` 为 `.core-gain-pop::after` 增加局部奖励光晕，普通收益浮层显示时播放 `coreGainPopHalo`。
- 过载收益浮层继续复用 `.core-gain-pop.is-overload-gain`，并在显示时播放更强的 `coreGainPopOverloadHalo`。
- 在 `prefers-reduced-motion: reduce` 中关闭该光晕动画。
- `tests/game.test.js` 增加静态断言，覆盖伪元素、动画名、keyframes 和降低动效兜底。
- 该改动只调整点火收益浮层展示层和测试，不新增可见文字、不新增收益、不新增存档字段，不改变点击收益、连击窗口、过载奖励、升级价格、星图路线、项目奖励、航线策略、航线指令、远航调度、反馈入口或部署链路。

验收标准：

- GitHub Issues 已同步：2026-05-02 00:54 CST 当前 5 个 open feedback issue、0 个 open bug issue；#5 作为本轮主处理对象。
- `src/styles.css` 包含 `coreGainPopHalo`、`coreGainPopOverloadHalo`、收益浮层光晕伪元素和降低动效兜底。
- `tests/game.test.js` 覆盖点火收益浮层命中奖励光晕静态绑定。
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认包含 `coreGainPopHalo`、`coreGainPopOverloadHalo`、收益浮层光晕伪元素和降低动效兜底。
- 发布验证已通过：提交 `bda8c65` 已推送到 `origin/main`；GitHub Pages workflow `25223929580` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `coreGainPopHalo`、`coreGainPopOverloadHalo` 和收益浮层光晕降低动效兜底。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火收益浮层命中奖励光晕、验证结果、Pages 部署和复测问题；issue 保持 open，更新时间为 2026-05-01T17:03:35Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4360539017`。
- 回复后同步 GitHub Issues：2026-05-02 01:03 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue。
...
## 2026-05-02 Product decision：点火阶段光环命中补光

阶段判断：仓库已有 package.json、可玩游戏、GitHub Pages 部署和游戏内反馈入口；GitHub Issues 2026-05-02 00:43 CST 已同步到 5 个 open feedback issue、0 个 open bug issue。没有新的 bug issue；本轮继续处理 #5“点火按钮太薄弱、增加点击反馈和点击欲望”。

当前最大问题：点火按钮已经有按住反冲、落点反馈、收益浮层、顶部读数亮闪、连击轨光扫、倒计时徽标跳闪和星核裂纹闪亮。阶段光环虽然会随 8 格连击进度升温，但普通点击命中时主要只更新角度，缺少“这一击推进了阶段环”的短促确认。

本轮决策：

- 新增“点火阶段光环命中补光”。
- `src/styles.css` 复用 `.core-button.is-pulsing:not(.is-overload-impact)`，让 `.core-stage-aura` 在普通点火命中时播放 `coreStageHitFlash`。
- 过载命中继续使用已有 `.core-stage-aura.is-overload-hit` 与 `coreStageBurst`，避免普通补光覆盖第 8 次爆发。
...
- GitHub Issues 已同步：2026-05-02 00:43 CST 当前 5 个 open feedback issue、0 个 open bug issue；#5 作为本轮主处理对象。
- `src/styles.css` 包含 `coreStageHitFlash`、普通命中阶段光环选择器和降低动效兜底。
...
- 发布验证已通过：提交 `7ef5c3c` 已推送到 `origin/main`；GitHub Pages workflow `25222925852` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `coreVeinHitFlash`、普通命中星核裂纹选择器和降低动效兜底。
- 已回复 GitHub Issue #5，说明点火星核裂纹命中闪亮、验证结果、Pages 部署和复测问题；issue 保持 open，更新时间为 2026-05-01T16:37:17Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4360403165`。
- 回复后同步 GitHub Issues：2026-05-02 00:37 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue。
...
- 发布验证已通过：提交 `2096962` 已推送到 `origin/main`；GitHub Pages workflow `25222310881` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `coreBadgeStepHit`、普通命中倒计时徽标选择器和降低动效兜底。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火倒计时徽标命中跳闪、验证结果、Pages 部署和复测问题；issue 保持 open，更新时间为 2026-05-01T16:21:05Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4360313519`。
- 回复后同步 GitHub Issues：2026-05-02 00:21 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue。
...
- 发布验证已通过：提交 `e2169bb` 已推送到 `origin/main`；GitHub Pages workflow `25221827391` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含 `coreComboTrackHitSweep`、`coreComboTrackOverloadSweep`、连击轨命中选择器和降低动效兜底。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火连击轨命中光扫、验证结果、Pages 部署和复测问题；issue 保持 open，更新时间为 2026-05-01T16:07:51Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4360247871`。
- 回复后同步 GitHub Issues：2026-05-02 00:08 CST 当前仍为 5 个 open feedback issue、0 个 open bug issue。
...
- 发布验证已通过：提交 `d2ac74a` 已推送到 `origin/main`；GitHub Pages workflow `25219797994` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/app.js` 与 `src/styles.css` 已确认包含点火能量读数命中亮闪代码和样式。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #5，说明点火能量读数命中亮闪、验证结果、Pages 部署和复测问题，issue 保持 open，更新时间为 2026-05-01T15:15:25Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/5#issuecomment-4359988907`。
...
- 发布验证已通过：提交 `cd7a676` 已推送到 `origin/main`；GitHub Pages workflow `25219249865` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含小屏 active-route 网格覆盖。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航对照条当前路线小屏宽位修正、验证结果、Pages 部署和复测问题，issue 保持 open，更新时间为 2026-05-01T15:00:24Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4359914839`。
- 已回复 GitHub Issue #4，说明本轮用小屏 active-route 网格覆盖降低窄屏文字挤压，issue 保持 open，更新时间为 2026-05-01T15:00:53Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4359917161`。
...
- 本地验证已通过：`node --test tests/game.test.js`、`bun install --no-save`、`bun run test`、`bun run build`、`npm install`、`npm test` 和 `npm run build`；测试数 118 项。
- 构建产物已确认包含当前路线宽位网格。
- 发布验证已通过：提交 `37f11ee` 已推送到 `origin/main`；GitHub Pages workflow `25218713323` 成功，build job 已执行 `npm install`、`npm test` 和 `npm run build`，deploy job 成功；线上首页返回 HTTP 200，线上 `src/styles.css` 已确认包含当前路线宽位网格。workflow 继续出现 Node.js 20 actions 弃用提醒，未影响本次部署。
- 已回复 GitHub Issue #6，说明远航对照条当前路线操作收益宽位、验证结果、Pages 部署和复测问题，issue 保持 open，更新时间为 2026-05-01T14:45:45Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/6#issuecomment-4359844607`。
- 已回复 GitHub Issue #4，说明本轮用当前路线内部宽位降低横向挤压和文字密度压力，issue 保持 open，更新时间为 2026-05-01T14:46:04Z；评论地址：`https://github.com/Jassy930/codex-game-operator/issues/4#issuecomment-4359846047`。
