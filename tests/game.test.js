...
test("航线指令会返回轮换目标提示", () => {
  ...
  assert.equal(
    readyTask.text,
    "航线委托 0/3 · 下一步 点火齐射或巡航回收 · 可执行 · 下一步收益 预案执行 +6%，完成 3/3 推荐轮换 · 完成奖励 委托完成 +8%"
  );
...
test("轮换航线指令会触发航线连携收益", () => {
  ...
  assert.equal(
    secondTask.text,
    "航线委托 2/3 · 下一步 谐振脉冲 · 可执行 · 下一步收益 连携 +24% · 轮换目标 +18% · 策略终结 +12% · 委托完成 +8%，完成 3/3 推荐轮换 · 完成奖励 委托完成 +8%"
  );
  assert.equal(
...
test("静态首页会渲染航线指令轮换目标", () => {
  ...
  assert.match(appJs, /elements\.directiveTask\.replaceChildren\(text, action, status, reward, meter\)/);
