...
test("星图项目会给中后段玩家新的可追目标", () => {
  ...
  assert.equal(projects[0].segmentTotal, 46);
  assert.equal(projects[0].segmentText, "航段 1/46");
  ...
  assert.equal(projects[1].segmentText, "航段 2/46");
...
test("星图项目会标记所属章节", () => {
  ...
  assert.equal(projects[13].chapterText, "远航长尾 1/33");
  assert.equal(projects[30].chapterName, "远航长尾");
  assert.equal(projects[30].chapterText, "远航长尾 18/33");
  assert.equal(projects[31].chapterName, "远航长尾");
  assert.equal(projects[31].chapterText, "远航长尾 19/33");
  assert.equal(projects[32].chapterName, "远航长尾");
  assert.equal(projects[32].chapterText, "远航长尾 20/33");
  assert.equal(projects[33].chapterName, "远航长尾");
  assert.equal(projects[33].chapterText, "远航长尾 21/33");
  assert.equal(projects[34].chapterName, "远航长尾");
  assert.equal(projects[34].chapterText, "远航长尾 22/33");
  assert.equal(projects[35].chapterName, "远航长尾");
  assert.equal(projects[35].chapterText, "远航长尾 23/33");
  assert.equal(projects[36].chapterName, "远航长尾");
  assert.equal(projects[36].chapterText, "远航长尾 24/33");
  assert.equal(projects[37].chapterName, "远航长尾");
  assert.equal(projects[37].chapterText, "远航长尾 25/33");
  assert.equal(projects[38].chapterName, "远航长尾");
  assert.equal(projects[38].chapterText, "远航长尾 26/33");
  assert.equal(projects[39].chapterName, "远航长尾");
  assert.equal(projects[39].chapterText, "远航长尾 27/33");
  assert.equal(projects[40].chapterName, "远航长尾");
  assert.equal(projects[40].chapterText, "远航长尾 28/33");
  assert.equal(projects[41].chapterName, "远航长尾");
  assert.equal(projects[41].chapterText, "远航长尾 29/33");
  assert.equal(projects[42].chapterName, "远航长尾");
  assert.equal(projects[42].chapterText, "远航长尾 30/33");
  assert.equal(projects[43].chapterName, "远航长尾");
  assert.equal(projects[43].chapterText, "远航长尾 31/33");
  assert.equal(projects[44].chapterName, "远航长尾");
  assert.equal(projects[44].chapterText, "远航长尾 32/33");
  assert.equal(projects[45].chapterName, "远航长尾");
  assert.equal(projects[45].chapterText, "远航长尾 33/33");
});
...
test("星图总览会显示完成数和下一段奖励", () => {
  ...
  assert.equal(overview.total, 46);
  ...
  assert.equal(
    ...
    "星图进度 1/46 · 下一段 2/46：谐振校准 · 奖励 过载奖励 +20%"
  );
  ...
  assert.equal(
    ...
    "阶段导航：首段星图 1/4 · 专精校准 0/5 · 深空基建 0/4 · 远航长尾 0/33；当前 首段星图 2/4 · 航段 2/46 谐振校准 · 本阶段还剩 3 段 · 下一阶段 专精校准"
  );
  assert.equal(
    overview.chapterTargetText,
    "章节目标：首段星图 2/4 谐振校准 · 专精校准 1/5 点火航校 · 深空基建 1/4 远星中继 · 远航长尾 1/33 星门远征"
  );
  assert.equal(
    overview.chapterRewardText,
    "章节奖励：首段星图 总产能 1 / 点击 1 / 自动 1 / 过载 1 · 专精校准 总产能 3 / 点击 1 / 自动 1 · 深空基建 总产能 1 / 点击 1 / 自动 2 · 远航长尾 总产能 9 / 点击 8 / 自动 8 / 过载 8"
  );
  assert.equal(
    overview.rewardProgressText,
    "奖励进度：总产能 1/14 · 点击 0/11 · 自动 0/12 · 过载 0/9"
  );
  assert.equal(
  ...
  assert.equal(
    ...
    "里程碑：本章终点 采集阵列（航段 4/46 · 自动产能 +18% · 进度 11 级 / 12 级 · 还差 1 级）；终局航点 星瀑谐振门（航段 46/46 · 过载奖励 +26% · 进度 114.4K 能量 / 4.9B 能量 · 还差 4.9B 能量）"
  );
  assert.equal(
    overview.routeFocusText,
  ...
  assert.equal(
    ...
    "航线构成：37 个累计航段 · 9 个升级航段 · 奖励分布 总产能 14 段 / 点击 11 段 / 自动 12 段 / 过载 9 段"
  );
...
test("星图总览在首个项目前会提示等待奖励生效", () => {
  ...
  assert.equal(
    overview.chapterTargetText,
    "章节目标：首段星图 1/4 点亮星图 · 专精校准 1/5 点火航校 · 深空基建 1/4 远星中继 · 远航长尾 1/33 星门远征"
  );
  assert.equal(
    overview.chapterRewardText,
    "章节奖励：首段星图 总产能 1 / 点击 1 / 自动 1 / 过载 1 · 专精校准 总产能 3 / 点击 1 / 自动 1 · 深空基建 总产能 1 / 点击 1 / 自动 2 · 远航长尾 总产能 9 / 点击 8 / 自动 8 / 过载 8"
  ...
  assert.equal(
    overview.rewardProgressText,
    "奖励进度：总产能 0/14 · 点击 0/11 · 自动 0/12 · 过载 0/9"
  );
  assert.equal(
    overview.rewardTargetText,
  ...
  assert.equal(
    overview.milestoneText,
    "里程碑：本章终点 采集阵列（航段 4/46 · 自动产能 +18% · 进度 0 级 / 12 级 · 还差 12 级）；终局航点 星瀑谐振门（航段 46/46 · 过载奖励 +26% · 进度 0 能量 / 4.9B 能量 · 还差 4.9B 能量）"
  );
...
test("100K 后当前目标会指向下一个未完成星图项目", () => {
  ...
  assert.equal(goal.segmentTotal, 46);
  assert.equal(goal.upgradeId, "resonator");
  assert.equal(
    goal.progressText,
    "航段 2/46 · 进度 0 级 / 6 级 · 可购买星核谐振器 · 奖励 过载奖励 +20%"
  );
});
...
test("星图主目标会直接显示项目奖励", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 4/46 · 进度 11 级 / 12 级 · 可购买自动采集臂 · 奖励 自动产能 +18%"
  );
});
...
test("首批星图项目完成后会继续指向策略专精目标", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 5/46 · 进度 12 级 / 14 级 · 可购买聚能透镜 · 奖励 点击产能 +16%"
  );
});
...
test("500K 后星图会继续指向深空矿带", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 11/46 · 进度 600K 能量 / 750K 能量 · 还差 150K 能量 · 奖励 点击产能 +26%"
  );
});
...
test("深空航段完成后会继续指向星门远征", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 14/46 · 进度 1.2M 能量 / 1.5M 能量 · 还差 300K 能量 · 奖励 总产能 +16%"
  );
});
...
test("星门远征完成后会继续指向外环信标", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 15/46 · 进度 1.6M 能量 / 2M 能量 · 还差 400K 能量 · 奖励 点击产能 +14%"
  );
});
...
test("外环信标完成后会继续指向彗尾船坞", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 16/46 · 进度 2.1M 能量 / 2.8M 能量 · 还差 650K 能量 · 奖励 自动产能 +18%"
  );
});
...
test("彗尾船坞完成后会继续指向引力航标", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 17/46 · 进度 3M 能量 / 3.5M 能量 · 还差 500K 能量 · 奖励 总产能 +15%"
  );
});
...
test("引力航标完成后会继续指向暗流观测站", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 18/46 · 进度 3.8M 能量 / 4.5M 能量 · 还差 700K 能量 · 奖励 过载奖励 +15%"
  );
});
...
test("暗流观测站完成后会继续指向虚空罗盘", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 19/46 · 进度 4.9M 能量 / 5.8M 能量 · 还差 850K 能量 · 奖励 点击产能 +15%"
  );
});
...
test("虚空罗盘完成后会继续指向星云档案库", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 20/46 · 进度 6.2M 能量 / 7.3M 能量 · 还差 1.1M 能量 · 奖励 自动产能 +17%"
  );
});
...
test("星云档案库完成后会继续指向深穹测距阵", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 21/46 · 进度 7.6M 能量 / 9M 能量 · 还差 1.4M 能量 · 奖励 总产能 +17%"
  );
});
...
test("深穹测距阵完成后会继续指向寂光中继环", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 22/46 · 进度 9.5M 能量 / 11M 能量 · 还差 1.5M 能量 · 奖励 过载奖励 +16%"
  );
});
...
test("寂光中继环完成后会继续指向极昼折跃井", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 23/46 · 进度 11.5M 能量 / 13.5M 能量 · 还差 2M 能量 · 奖励 点击产能 +16%"
  );
});
...
test("极昼折跃井完成后会继续指向日冕回廊", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 24/46 · 进度 14M 能量 / 16.5M 能量 · 还差 2.5M 能量 · 奖励 自动产能 +16%"
  );
});
...
test("日冕回廊完成后会继续指向磁帆穹顶", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 25/46 · 进度 17M 能量 / 20M 能量 · 还差 3M 能量 · 奖励 总产能 +18%"
  );
});
...
test("磁帆穹顶完成后会继续指向引潮星环", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 26/46 · 进度 21M 能量 / 24M 能量 · 还差 3M 能量 · 奖励 过载奖励 +18%"
  );
});
...
test("引潮星环完成后会继续指向脉冲航闸", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 27/46 · 进度 25M 能量 / 30M 能量 · 还差 5M 能量 · 奖励 点击产能 +18%"
  );
});
...
test("脉冲航闸完成后会继续指向离辉轨道港", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 28/46 · 进度 31M 能量 / 38M 能量 · 还差 7M 能量 · 奖励 自动产能 +18%"
  );
});
...
test("离辉轨道港完成后会继续指向远曦灯塔", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 29/46 · 进度 39M 能量 / 50M 能量 · 还差 11M 能量 · 奖励 总产能 +19%"
  );
});
...
test("远曦灯塔完成后会继续指向星冠回声站", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 30/46 · 进度 52M 能量 / 65M 能量 · 还差 13M 能量 · 奖励 过载奖励 +19%"
  );
});
...
test("星冠回声站完成后会继续指向晨渊观星台", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 31/46 · 进度 66M 能量 / 85M 能量 · 还差 19M 能量 · 奖励 点击产能 +19%"
  );
});
...
test("晨渊观星台完成后会继续指向曜幕天穹", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 32/46 · 进度 86M 能量 / 110M 能量 · 还差 24M 能量 · 奖励 自动产能 +20%"
  );
});
...
test("曜幕天穹完成后会继续指向星渊回声门", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 33/46 · 进度 111M 能量 / 145M 能量 · 还差 34M 能量 · 奖励 总产能 +20%"
  );
});
...
test("星渊回声门完成后会继续指向虚曜谐振塔", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 34/46 · 进度 146M 能量 / 190M 能量 · 还差 44M 能量 · 奖励 过载奖励 +20%"
  );
});
...
test("虚曜谐振塔完成后会继续指向烁影星匣", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 35/46 · 进度 191M 能量 / 250M 能量 · 还差 59M 能量 · 奖励 点击产能 +21%"
  );
});
...
test("烁影星匣完成后会继续指向辉潮巡航环", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 36/46 · 进度 251M 能量 / 320M 能量 · 还差 69M 能量 · 奖励 自动产能 +21%"
  );
});
...
test("辉潮巡航环完成后会继续指向星澜汇流庭", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 37/46 · 进度 321M 能量 / 420M 能量 · 还差 99M 能量 · 奖励 总产能 +22%"
  );
});
...
test("星澜汇流庭完成后会继续指向穹海棱镜", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 38/46 · 进度 421M 能量 / 550M 能量 · 还差 129M 能量 · 奖励 过载奖励 +22%"
  );
});
...
test("穹海棱镜完成后会继续指向极光谱站", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 39/46 · 进度 551M 能量 / 720M 能量 · 还差 169M 能量 · 奖励 点击产能 +23%"
  );
});
...
test("极光谱站完成后会继续指向银翼曙环", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 40/46 · 进度 721M 能量 / 950M 能量 · 还差 229M 能量 · 奖励 自动产能 +23%"
  );
});
...
test("银翼曙环完成后会继续指向晨星方舟", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 41/46 · 进度 951M 能量 / 1.2B 能量 · 还差 249M 能量 · 奖励 总产能 +24%"
  );
});
...
test("晨星方舟完成后会继续指向星潮谐振庭", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 42/46 · 进度 1.2B 能量 / 1.6B 能量 · 还差 399M 能量 · 奖励 过载奖励 +24%"
  );
});
...
test("星潮谐振庭完成后会继续指向星辉远港", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 43/46 · 进度 1.6B 能量 / 2.1B 能量 · 还差 499M 能量 · 奖励 点击产能 +25%"
  );
});
...
test("星辉远港完成后会继续指向辉翼巡航港", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 44/46 · 进度 2.1B 能量 / 2.8B 能量 · 还差 699M 能量 · 奖励 自动产能 +25%"
  );
});
...
test("辉翼巡航港完成后会继续指向星穹导航塔", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 45/46 · 进度 2.8B 能量 / 3.7B 能量 · 还差 899M 能量 · 奖励 总产能 +26%"
  );
});
...
test("星穹导航塔完成后会继续指向星瀑谐振门", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 46/46 · 进度 3.7B 能量 / 4.9B 能量 · 还差 1.2B 能量 · 奖励 过载奖励 +26%"
  );
});
...
test("星图总览会显示全部完成状态", () => {
  ...
  assert.equal(overview.completed, 46);
  assert.equal(overview.total, 46);
  assert.equal(overview.nextProjectId, null);
  assert.equal(overview.summaryText, "星图进度 46/46 · 全部航段已完成");
  assert.equal(
    overview.detailText,
  ...
  assert.equal(
    overview.chapterText,
    "阶段导航：首段星图 4/4 · 专精校准 5/5 · 深空基建 4/4 · 远航长尾 33/33；全部阶段完成"
  );
  assert.equal(
    overview.chapterTargetText,
  ...
  assert.equal(
    overview.rewardProgressText,
    "奖励进度：总产能 14/14 · 点击 11/11 · 自动 12/12 · 过载 9/9"
  );
  assert.equal(
    overview.rewardTargetText,
  ...
  assert.equal(
    overview.bonusText,
    "生效加成：总产能 x11.52 · 点击 x6.85 · 自动 x9.31 · 过载 x5.14"
  );
...
test("星图阶段导航会显示最终阶段余量", () => {
  ...
  assert.equal(
    overview.chapterText,
    "阶段导航：首段星图 4/4 · 专精校准 5/5 · 深空基建 4/4 · 远航长尾 17/33；当前 远航长尾 18/33 · 航段 31/46 晨渊观星台 · 本阶段还剩 16 段 · 最终阶段"
  );
});
