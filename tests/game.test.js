...
test("星图筛选摘要会显示奖励构成、下一条和终点航段", () => {
  ...
  assert.equal(
    ...
    "筛选视图：本章 5 段 · 已完成 1/5 · 奖励构成 总产能 3 段 / 点击 1 段 / 自动 1 段 · 下一条 航段 5/57 点火航校（点击产能 +16% · 进度 12 级 / 14 级 · 还差 2 级）；后续 航段 6/57 巡航航校、航段 8/57 稳定矩阵；终点 航段 9/57 均衡校准"
  );
  assert.equal(
    getProjectFilterSummary(active, "upgrade-track"),
    "筛选视图：升级 9 段 · 已完成 3/9 · 奖励构成 总产能 3 段 / 点击 2 段 / 自动 3 段 / 过载 1 段 · 下一条 航段 5/57 点火航校（点击产能 +16% · 进度 12 级 / 14 级 · 还差 2 级）；后续 航段 6/57 巡航航校、航段 8/57 稳定矩阵；终点 航段 13/57 恒星锚点"
  );
  assert.equal(
    getProjectFilterSummary(active, "current"),
    "筛选视图：当前 1 段 · 已完成 0/1 · 奖励构成 点击 1 段 · 下一条 航段 5/57 点火航校（点击产能 +16% · 进度 12 级 / 14 级 · 还差 2 级）"
  );
  assert.equal(
    getProjectFilterSummary(active, "completed"),
    "筛选视图：已完成 5 段 · 全部已完成 · 奖励构成 总产能 2 段 / 点击 1 段 / 自动 1 段 / 过载 1 段"
  );
  assert.equal(
