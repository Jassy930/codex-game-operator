...
test("航线指令会返回轮换目标提示", () => {
  ...
  const waitingTask = getDirectiveTaskStatus(
    {
      ...createInitialState(0),
      totalEnergy: 100_000,
      directives: {
  ...
  assert.equal(lockedTask.nextStepText, "");
  assert.equal(lockedTask.nextIntentText, "");
  assert.equal(lockedTask.nextActionText, "");
  ...
  assert.equal(lockedTask.completedFollowupText, "");
  ...
  assert.equal(readyTask.nextStepText, "第 1/3 步");
  assert.equal(readyTask.nextIntentText, "收束起手");
  assert.equal(readyTask.nextActionText, "下一步 点火齐射或巡航回收");
  ...
  assert.equal(readyTask.completedFollowupText, "");
  assert.equal(
    readyTask.text,
    "航线委托 0/3 · 第 1/3 步 · 收束起手 · 下一步 点火齐射或巡航回收 · 可执行 · 下一步收益 预案执行 +6%，完成 3/3 推荐轮换 · 完成奖励 委托完成 +8%"
  );
  assert.equal(waitingTask.nextStepText, "第 1/3 步");
  assert.equal(waitingTask.nextIntentText, "收束起手");
  assert.equal(waitingTask.nextActionText, "下一步 点火齐射或巡航回收");
  assert.equal(waitingTask.nextStatusText, "等待 34 秒");
  assert.equal(waitingTask.nextStatusKind, "waiting");
});
...
test("轮换航线指令会触发航线连携收益", () => {
  ...
  const firstTask = getDirectiveTaskStatus(first.state, 2000);
  ...
  assert.equal(firstTask.nextIntentText, "收束续航");
  assert.equal(
    firstPlan.text,
  ...
  assert.equal(
    secondTask.text,
    "航线委托 2/3 · 第 3/3 步 · 策略终结 · 下一步 谐振脉冲 · 可执行 · 下一步收益 连携 +24% · 轮换目标 +18% · 策略终结 +12% · 委托完成 +8%，完成 3/3 推荐轮换 · 完成奖励 委托完成 +8%"
  );
  assert.equal(
    secondTask.nextRewardText,
    "连携 +24% · 轮换目标 +18% · 策略终结 +12% · 委托完成 +8%"
  );
  assert.equal(secondTask.nextActionText, "下一步 谐振脉冲");
  assert.equal(secondTask.nextStepText, "第 3/3 步");
  assert.equal(secondTask.nextIntentText, "策略终结");
  assert.equal(secondTask.nextStatusText, "可执行");
  assert.equal(secondTask.nextStatusKind, "ready");
  ...
  assert.equal(continuationTask.nextStepText, "");
  assert.equal(continuationTask.nextIntentText, "");
  assert.equal(continuationTask.nextActionText, "");
  ...
  assert.equal(continuationTask.completedFollowupText, "熟练续航 点火齐射");
...
test("点火按钮会渲染点击反馈和过载前兆效果", () => {
  ...
  assert.match(indexHtml, /id="coreImpactFlare" class="core-impact-flare"/);
  assert.match(indexHtml, /id="coreImpactEcho" class="core-impact-echo"/);
  ...
  assert.match(appJs, /elements\.coreImpactFlare\.classList\.add\("is-showing"\)/);
  ...
  assert.match(styles, /\.core-impact-flare/);
  assert.match(styles, /\.core-impact-flare\.is-showing/);
  assert.match(styles, /\.core-impact-flare\.is-overload-impact/);
  assert.match(styles, /@keyframes coreImpactFlare/);
  assert.match(styles, /\.core-impact-echo/);
  ...
  assert.match(styles, /\.core-gain-trail::before,\n\.core-gain-trail::after/);
  assert.match(styles, /\.core-gain-trail::before/);
  assert.match(styles, /\.core-gain-trail::after/);
  assert.match(styles, /\.core-gain-trail\.is-showing/);
  assert.match(styles, /\.core-gain-trail\.is-showing::before,\n\.core-gain-trail\.is-showing::after/);
  assert.match(styles, /\.core-gain-trail\.is-overload-gain/);
  assert.match(styles, /\.core-gain-trail\.is-overload-gain::before/);
  assert.match(styles, /\.core-gain-trail\.is-overload-gain::after/);
  assert.match(styles, /\.core-gain-trail\.is-overload-gain\.is-showing::before,\n\.core-gain-trail\.is-overload-gain\.is-showing::after/);
  assert.match(styles, /@keyframes coreGainTrail/);
  assert.match(styles, /@keyframes coreGainTrailSpark/);
  ...
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\) \{[\s\S]*\.core-gain-trail\.is-showing::before[\s\S]*animation: none/);
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\) \{[\s\S]*\.core-gain-trail\.is-showing::after[\s\S]*animation: none/);
...
test("静态首页会渲染航线指令轮换目标", () => {
  ...
  assert.match(indexHtml, /class="directive-task-step" hidden/);
  ...
  assert.match(indexHtml, /class="directive-task-meter-node is-pending" aria-hidden="true">2<\/span>/);
  assert.match(indexHtml, /class="directive-task-meter-node is-pending" aria-hidden="true">3<\/span>/);
  ...
  assert.match(appJs, /const actionText = task\.completed \? task\.completedFollowupText : task\.nextActionText/);
  assert.match(appJs, /"directive-task-action",/);
  assert.match(appJs, /action\.textContent = actionText \?\? ""/);
  assert.match(appJs, /task\.completed \? "航线委托完成续航动作：" : "航线委托下一步动作："/);
  ...
  assert.match(gameJs, /function buildDirectiveTaskNextIntentText/);
  assert.match(gameJs, /function buildDirectiveTaskNextStatus/);
  assert.match(gameJs, /function getDirectiveRemainingMs/);
  ...
  assert.match(styles, /\.directive-task-action\.is-completed/);
  ...
  assert.match(styles, /\.directive-task-status\[hidden\]/);
  ...
  assert.match(styles, /\.directive-task\.is-completed \.directive-task-meter-fill::after/);
  assert.match(styles, /@keyframes directiveTaskCompletedMeterSweep/);
  ...
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\) \{[\s\S]*\.directive-task\.is-completed \.directive-task-meter-fill::after[\s\S]*animation: none/);
  ...
  assert.match(styles, /\.far-dispatch-branch-choice-summary-item\.is-active-route[\s\S]*\.far-dispatch-branch-choice-summary-progress::after[\s\S]*6px 0 12px rgba\(255, 211, 106, 0\.22\)/);
  assert.match(styles, /\.far-dispatch-branch-choice-summary-item\.is-active-route[\s\S]*\.far-dispatch-branch-choice-summary-progress\.is-detour::after[\s\S]*border-right-color: rgba\(255, 209, 223, 0\.76\)/);
  assert.match(styles, /\.far-dispatch-branch-choice-summary-item\.is-active-route[\s\S]*\.far-dispatch-branch-choice-summary-progress\.is-detour::after[\s\S]*6px 0 12px rgba\(255, 126, 177, 0\.22\)/);
  assert.match(styles, /\.far-dispatch-branch-choice-summary-progress-node\.is-done/);
  assert.match(styles, /\.far-dispatch-branch-choice-summary-item\.is-active-route[\s\S]*\.far-dispatch-branch-choice-summary-progress-node\.is-done/);
  assert.match(styles, /\.far-dispatch-branch-choice-summary-item\.is-active-route[\s\S]*\.far-dispatch-branch-choice-summary-progress-node\.is-done::after/);
  assert.match(styles, /\.far-dispatch-branch-choice-summary-progress\.is-detour[\s\S]*\.far-dispatch-branch-choice-summary-progress-node\.is-done::after/);
  assert.match(styles, /\.far-dispatch-branch-choice-summary-item\.is-active-route[\s\S]*\.far-dispatch-branch-choice-summary-progress-node\.is-waiting/);
  assert.match(styles, /\.far-dispatch-branch-choice-summary-item\.is-active-route[\s\S]*\.far-dispatch-branch-choice-summary-progress-node\.is-waiting::after/);
  assert.match(styles, /\.far-dispatch-branch-choice-summary-progress\.is-detour[\s\S]*\.far-dispatch-branch-choice-summary-progress-node\.is-waiting::after/);
  ...
  assert.match(styles, /@keyframes farDispatchSummaryActiveRouteRailBeacon/);
  assert.match(styles, /@keyframes farDispatchSummaryDoneNodeEcho/);
  assert.match(styles, /@keyframes farDispatchSummaryWaitingNodeBeacon/);
  ...
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\) \{[\s\S]*\.far-dispatch-branch-choice-summary-progress::before[\s\S]*animation: none/);
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\) \{[\s\S]*\.far-dispatch-branch-choice-summary-progress-node\.is-done::after[\s\S]*animation: none/);
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\) \{[\s\S]*\.far-dispatch-branch-choice-summary-progress-node\.is-waiting::after[\s\S]*animation: none/);
