...
test("星图项目会给中后段玩家新的可追目标", () => {
  ...
  assert.equal(projects[0].segmentTotal, 16);
  assert.equal(projects[0].segmentText, "航段 1/16");
  ...
  assert.equal(projects[1].segmentText, "航段 2/16");
...
test("星图总览会显示完成数和下一段奖励", () => {
  ...
  assert.equal(
    ...
    "星图进度 1/16 · 下一段 2/16：谐振校准 · 奖励 过载奖励 +20%"
  );
...
test("100K 后当前目标会指向下一个未完成星图项目", () => {
  ...
  assert.equal(goal.segmentTotal, 16);
  assert.equal(goal.upgradeId, "resonator");
  assert.equal(
    goal.progressText,
    "航段 2/16 · 进度 0 级 / 6 级 · 可购买星核谐振器 · 奖励 过载奖励 +20%"
  );
});
...
test("星图主目标会直接显示项目奖励", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 4/16 · 进度 11 级 / 12 级 · 可购买自动采集臂 · 奖励 自动产能 +18%"
  );
});
...
test("首批星图项目完成后会继续指向策略专精目标", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 5/16 · 进度 12 级 / 14 级 · 可购买聚能透镜 · 奖励 点击产能 +16%"
  );
});
...
test("500K 后星图会继续指向深空矿带", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 11/16 · 进度 600K 能量 / 750K 能量 · 还差 150K 能量 · 奖励 点击产能 +26%"
  );
});
...
test("深空航段完成后会继续指向星门远征", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 14/16 · 进度 1.2M 能量 / 1.5M 能量 · 还差 300K 能量 · 奖励 总产能 +16%"
  );
});
...
test("星门远征完成后会继续指向外环信标", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 15/16 · 进度 1.6M 能量 / 2M 能量 · 还差 400K 能量 · 奖励 点击产能 +14%"
  );
});
...
test("外环信标完成后会继续指向彗尾船坞", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 16/16 · 进度 2.1M 能量 / 2.8M 能量 · 还差 650K 能量 · 奖励 自动产能 +18%"
  );
});
...
test("星图总览会显示全部完成状态", () => {
  ...
  assert.equal(overview.completed, 16);
  assert.equal(overview.total, 16);
  assert.equal(overview.nextProjectId, null);
  assert.equal(overview.summaryText, "星图进度 16/16 · 全部航段已完成");
  assert.equal(
    overview.detailText,
  ...
  assert.equal(
    overview.bonusText,
    "生效加成：总产能 x2.67 · 点击 x1.97 · 自动 x2.60 · 过载 x1.20"
  );
