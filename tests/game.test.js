...
test("星图项目会给中后段玩家新的可追目标", () => {
  ...
  assert.equal(projects[0].segmentTotal, 26);
  assert.equal(projects[0].segmentText, "航段 1/26");
  ...
  assert.equal(projects[1].segmentText, "航段 2/26");
...
test("星图总览会显示完成数和下一段奖励", () => {
  ...
  assert.equal(overview.total, 26);
  ...
  assert.equal(
    ...
    "星图进度 1/26 · 下一段 2/26：谐振校准 · 奖励 过载奖励 +20%"
  );
...
test("100K 后当前目标会指向下一个未完成星图项目", () => {
  ...
  assert.equal(goal.segmentTotal, 26);
  assert.equal(goal.upgradeId, "resonator");
  assert.equal(
    goal.progressText,
    "航段 2/26 · 进度 0 级 / 6 级 · 可购买星核谐振器 · 奖励 过载奖励 +20%"
  );
});
...
test("星图主目标会直接显示项目奖励", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 4/26 · 进度 11 级 / 12 级 · 可购买自动采集臂 · 奖励 自动产能 +18%"
  );
});
...
test("首批星图项目完成后会继续指向策略专精目标", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 5/26 · 进度 12 级 / 14 级 · 可购买聚能透镜 · 奖励 点击产能 +16%"
  );
});
...
test("500K 后星图会继续指向深空矿带", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 11/26 · 进度 600K 能量 / 750K 能量 · 还差 150K 能量 · 奖励 点击产能 +26%"
  );
});
...
test("深空航段完成后会继续指向星门远征", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 14/26 · 进度 1.2M 能量 / 1.5M 能量 · 还差 300K 能量 · 奖励 总产能 +16%"
  );
});
...
test("星门远征完成后会继续指向外环信标", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 15/26 · 进度 1.6M 能量 / 2M 能量 · 还差 400K 能量 · 奖励 点击产能 +14%"
  );
});
...
test("外环信标完成后会继续指向彗尾船坞", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 16/26 · 进度 2.1M 能量 / 2.8M 能量 · 还差 650K 能量 · 奖励 自动产能 +18%"
  );
});
...
test("彗尾船坞完成后会继续指向引力航标", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 17/26 · 进度 3M 能量 / 3.5M 能量 · 还差 500K 能量 · 奖励 总产能 +15%"
  );
});
...
test("引力航标完成后会继续指向暗流观测站", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 18/26 · 进度 3.8M 能量 / 4.5M 能量 · 还差 700K 能量 · 奖励 过载奖励 +15%"
  );
});
...
test("暗流观测站完成后会继续指向虚空罗盘", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 19/26 · 进度 4.9M 能量 / 5.8M 能量 · 还差 850K 能量 · 奖励 点击产能 +15%"
  );
});
...
test("虚空罗盘完成后会继续指向星云档案库", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 20/26 · 进度 6.2M 能量 / 7.3M 能量 · 还差 1.1M 能量 · 奖励 自动产能 +17%"
  );
});
...
test("星云档案库完成后会继续指向深穹测距阵", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 21/26 · 进度 7.6M 能量 / 9M 能量 · 还差 1.4M 能量 · 奖励 总产能 +17%"
  );
});
...
test("深穹测距阵完成后会继续指向寂光中继环", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 22/26 · 进度 9.5M 能量 / 11M 能量 · 还差 1.5M 能量 · 奖励 过载奖励 +16%"
  );
});
...
test("寂光中继环完成后会继续指向极昼折跃井", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 23/26 · 进度 11.5M 能量 / 13.5M 能量 · 还差 2M 能量 · 奖励 点击产能 +16%"
  );
});
...
test("极昼折跃井完成后会继续指向日冕回廊", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 24/26 · 进度 14M 能量 / 16.5M 能量 · 还差 2.5M 能量 · 奖励 自动产能 +16%"
  );
});
...
test("日冕回廊完成后会继续指向磁帆穹顶", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 25/26 · 进度 17M 能量 / 20M 能量 · 还差 3M 能量 · 奖励 总产能 +18%"
  );
});
...
test("磁帆穹顶完成后会继续指向引潮星环", () => {
  ...
  assert.equal(
    goal.progressText,
    "航段 26/26 · 进度 21M 能量 / 24M 能量 · 还差 3M 能量 · 奖励 过载奖励 +18%"
  );
});
...
test("星图总览会显示全部完成状态", () => {
  ...
  assert.equal(overview.completed, 26);
  assert.equal(overview.total, 26);
  assert.equal(overview.nextProjectId, null);
  assert.equal(overview.summaryText, "星图进度 26/26 · 全部航段已完成");
  assert.equal(
    overview.detailText,
  ...
  assert.equal(
    overview.bonusText,
    "生效加成：总产能 x4.23 · 点击 x2.62 · 自动 x3.53 · 过载 x1.89"
  );
