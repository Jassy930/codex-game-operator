...
test("远航调度会在 20M 后按当前航段指定目标指令", () => {
  ...
  assert.deepEqual(
    ...
    [
      "sync:谐振脉冲:可选择:补当前资源:推荐原因：点击/过载航段保留当前资源:路线目标：按当前航段推荐建立协同路线:后续协同回航触发闭环与远航突破:本步合计 +13% · 回目标 远航闭环 +16% + 远航突破 +0.05%剩余 + 契合闭环 +7%:远航协同 +5% · 协同补给 +3%当前 · 航段契合 +5%:focused=true",
      "detour:巡航回收:可选择:投送累计航段::路线目标：建立绕行路线，记录为下轮对照:后续绕行回航触发闭环与绕行突破:本步合计 +4% · 投送累计 · 回目标 远航闭环 +16% + 远航突破 +0.05%剩余 + 绕行突破 +0.03%剩余:远航绕行 +4% · 绕行投送 -0.3%当前:focused=false"
    ]
  );
  assert.equal(
    dispatch.branchChoiceText,
    "分支选择：协同 谐振脉冲（可选择 · 补当前资源 · 推荐原因：点击/过载航段保留当前资源 · 路线目标：按当前航段推荐建立协同路线 · 后续协同回航触发闭环与远航突破 · 本步合计 +13% · 回目标 远航闭环 +16% + 远航突破 +0.05%剩余 + 契合闭环 +7% · 远航协同 +5% · 协同补给 +3%当前 · 航段契合 +5%） / 绕行 巡航回收（可选择 · 投送累计航段 · 路线目标：建立绕行路线，记录为下轮对照 · 后续绕行回航触发闭环与绕行突破 · 本步合计 +4% · 投送累计 · 回目标 远航闭环 +16% + 远航突破 +0.05%剩余 + 绕行突破 +0.03%剩余 · 远航绕行 +4% · 绕行投送 -0.3%当前）"
  );
  assert.equal(
